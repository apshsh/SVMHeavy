...
double FuncVector::abs0(void) const
{
    NiceAssert( !ismixed() );
...
{ 
    ...
    {
        NiceThrow("Mixed functional vector multiplication is ill-defined");
    } 
...
void FuncVector::sample(int Nsamp) const
{
    NiceAssert( Nsamp > 0 );
    NiceAssert( dim() == 1 );
...
gentype &RKHSVector::innerp(gentype &res, const Vector<const Vector<gentype> *> &bb) const
{
    ...
    if ( m() == 1 )
    {
        ...
        return (*(bb(0))).innerp(res,bb(1,1,bb.size()-1,tmpva));
    }
...
double &RKHSVector::innerpReal(double &res, const Vector<const Vector<gentype> *> &bb) const
{
    ...
    if ( m() == 1 )
    {
        ...
        return (*(bb(0))).innerpReal(res,bb(1,1,bb.size()-1,tmpva));
    }
...
double &RKHSVector::baseinner2Real(double &res, const Vector<gentype> &bb, int conj, int aind, int bind) const
{
    ...
    for ( i = 0 ; i < N() ; ++i )
    {
        ...
        for ( j = 0 ; j < b.N() ; ++j )
        {
            ...
            tmpa *= spKern.K2(x(i),b.x(j),iinfo,jinfo,tmpc);
            tmpa *= tmpd;
    ...
    for ( i = 0 ; i < N() ; ++i )
    {
        ...
        for ( j = 0 ; j < b.N() ; ++j )
        {
            ...
            for ( k = 0 ; k < c.N() ; ++k )
            {
                ...
                tmpa *= spKern.K3(x(i),b.x(j),c.x(k),iinfo,jinfo,kinfo,tmpc);
                tmpa *= tmpd;
                tmpa *= tmpe;
        ...
        for ( j = 0 ; j < b.N() ; ++j )
        {
            ...
            for ( k = 0 ; k < c.N() ; ++k )
            {
                ...
                for ( l = 0 ; l < d.N() ; ++l )
                {
                    ...
                    tmpa *= spKern.K4(x(i),b.x(j),c.x(k),d.x(l),iinfo,jinfo,kinfo,linfo,tmpc);
                    tmpa *= tmpd;
                    tmpa *= tmpe;
                    tmpa *= tmpf;
...
gentype &BernVector::operator()(gentype &res, const SparseVector<gentype> &xx) const
{
    ...
    {
        NiceThrow("Multi-dimensional Bernstein polynomials require multiple weight vectors, and I don't have time to implement that right now - see http://www.iue.tuwien.ac.at/phd/heitzinger/node17.html");
    }
...
std::ostream &RKHSVector::outstream(std::ostream &output) const
{
    ...
    {
        output << "[[ RKHS kernel: " << getspKern()        << "\n";
        output << "   RKHS x:      " << getxx()            << "\n";
        output << "   RKHS a:      " << getalpha()         << "\n";
        output << "   RKHS m:      " << mm                 << "\n";
        output << "   RKHS aasv:   " << gettreatalphaasvector() << " ]]";
    }
...
/*
    if ( type() == 2 )
    {
        // res is an RKHS Vector

        dynamic_cast<RKHSVector &>(res).resetit(kern,x,xinfo,a,alphaasvec,m);
    }

    else if ( ( type() == 0 ) && res.imoverhere && ( (*(res.imoverhere)).type() == 2 ) )
    {
        // res is an indirected RKHS Vector

        dynamic_cast<RKHSVector &>(*(res.imoverhere)).resetit(kern,x,xinfo,a,alphaasvec,m);
    }

    else if ( ( type() == 0 ) && res.imoverhere )
    {
        // res is an indirected Non-RKHS Vector

        MEMDEL(res.imoverhere);

        RKHSVector *tmp;

        MEMNEW(tmp,RKHSVector);

        (*tmp).resetit(kern,x,xinfo,a,alphaasvec,m);
    ...
    {
        // res is a non-finite non-rkhs vector
