...
# dos/djgpp version - no threads but has keyboard interupts
...
#RM = rm -f
#MLMUTATEFILE = POSTSIG.TXT
##CBASEFLAGS = -DSPECIFYSYSVIAMAKE -DCYGWIN_BUILD -DHAVE_TERMIOS -DHAVE_IOCTL -DIS_CPP11 -std=c++11 
#CBASEFLAGS = -DCYGWIN10 -DSPECIFYSYSVIAMAKE -DCYGWIN_BUILD -DHAVE_TERMIOS -DIS_CPP11 -DHAVE_IOCTL -std=c++11 -Wno-strict-overflow
#BIGFLAG = -Wa,-mbig-obj
#LIBFLAGS = -lm
...
#RM = rm -f
#MLMUTATEFILE = POSTSIG.TXT
#CBASEFLAGS = -DSPECFN_ASUPP -DSPECIFYSYSVIAMAKE -DVISUAL_STU -DVISUAL_STU_OLD -DVISUAL_STU_NOERF -DHAVE_CONIO
#BIGFLAG = -big-obj
#LIBFLAGS = -lm
...
pyoptmathprof: CFLAGS = $(BASEFLAGS) $(CBASEFLAGS) $(OPTFLAGS) $(OPTFLAGSMATHS) $(PYTHONI) $(PYFLAGS) $(PROFFLAGS) -DPYLOCAL
pyoptmathprof: $(OBJS)
	$(CCC) $(PYTHONI) $(PYTARGSRC) -o $(PYTARGNAME) $(OBJS) $(CFLAGS) $(LIBFLAGS) $(PYTHONL) $(PROFFLAGS)
...
smatrix.o    : smatrix.cc smatrix.hpp $(SMATRIXDEP)
	$(CC) $< -o $@ -c $(CFLAGS)
gentype.o    : gentype.cc gentype.hpp $(GENTYPEDEP) plotbase.hpp
	$(CC) $< -o $@ -c $(CFLAGS)
