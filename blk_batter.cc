...
public:
    ...
    battSim(double _tdelta, double _theta = DEFAULT_BATTTHETA, double _qQ_e = -1, double _qSOC = -1, int _assumeDischarge = 0)
    {
        NiceAssert( ( _qQ_e < 0 ) || ( _qSOC < 0 ) );
        ...
        battStart(tdelta,_theta,VCHARGED,_qQ_e,_qSOC);

        return;
    }

    battSim(double Vstart, double _tdelta, double _theta, double _qQ_e, double _qSOC,
            double _qC_0star, double _qK_c, double _theta_f, double _epsilon, double _delta, double _qIstar,
            double _qE_m0, double _qK_E, double _tau_1, double _qR_00, double _qR_10, double _qA_0, double _qR_20, double _qA_21, double _qA_22,
            double _qE_p, double _qV_p0, double _qA_p, double _qG_p0,
            double _qC_theta, double _qR_theta, 
            int _assumeDischarge)
    {
        ...
        C_0star = _qC_0star*60*60; // conversion Ah -> coulombs
        K_c     = _qK_c;
        theta_f = _theta_f;
        epsilon = _epsilon;
        delta   = _delta;
        ...
        battStart(tdelta,_theta,Vstart,_qQ_e,_qSOC);
    ...
    void battStart(double _tdelta, double _theta, double Vstart, double _qQ_e = -1, double _qSOC = -1)
    {
...
*/
                ...
                if ( testisvnan(simuBatt.V) || testisvnan(simuBatt.I) || testisvnan(simuBatt.theta) )
                {
errstream() << "!!!" << t << "!!!";
                    res = 2;
                    break;
...
//errstream() << "phantomxqq 0\n";
        ...
        default:
        {
            NiceThrow("Mode unknown in BLK_Batter evaluation");
            break;
        }
    }
...
//errstream() << "phantomxyzqq 7\n";
        if ( ( runlen == maxrunlen ) && ( testisvnan(simuBatt.V) || testisvnan(simuBatt.I) || testisvnan(simuBatt.theta) ) )
        {
