...
//       - startpoint = 0: start training from current state (revert to 1 if globSolTol() == 0)
//                      1: start training from reset state
//                      2: start training from current state, only do one "cross" (effectively use first batch as a single validation set, revert to 3 if globSolTol() == 0)
//                      3: start training from reset state, only do one "cross" (effectively use first batch as a single validation set)
//                      4: start training from current state
//                      5: start training from reset state
//                      6: start training from current state, only do one "cross" (effectively use first batch as a single validation set)
//                      7: start training from reset state, only do one "cross" (effectively use first batch as a single validation set)
...
double calcCross   (const ML_Base &baseML, int m, int rndit = 0, int numreps = 1,                             int startpoint = 0, int suppressfb = 0, int useThreads = 0);
...
double calcSparSens(const ML_Base &baseML,                   Vector<double> &repres, Vector<double> &cnt, Matrix<double> &cfm, int minbad, int maxbad, double noisemean = 0, double noisevar = 0, int startpoint = 0, int suppressfb = 0, int useThreads = 0);

double calcLOO     (const ML_Base &baseML,                                           Vector<int>    &cnt, Matrix<int>    &cfm,        Vector<gentype>   &resh,        Vector<gentype>   &resg,        Vector<gentype>   &gvarres,                                                                    int startpoint = 0, int calcvarres = 0, int suppressfb = 0, int useThreads = 0);
double calcRecall  (const ML_Base &baseML,                                           Vector<int>    &cnt, Matrix<int>    &cfm,        Vector<gentype>   &resh,        Vector<gentype>   &resg,        Vector<gentype>   &gvarres,                                                                    int startpoint = 0, int calcvarres = 0, int suppressfb = 0, int useThreads = 0);
double calcCross   (const ML_Base &baseML, int m, int rndit, Vector<double> &repres, Vector<double> &cnt, Matrix<double> &cfm, Vector<Vector<gentype> > &resh, Vector<Vector<gentype> > &resg, Vector<Vector<gentype> > &gvarres, int numreps = 1,                                                   int startpoint = 0, int calcvarres = 0, int suppressfb = 0, int useThreads = 0);
double calcSparSens(const ML_Base &baseML,                   Vector<double> &repres, Vector<double> &cnt, Matrix<double> &cfm, Vector<Vector<gentype> > &resh, Vector<Vector<gentype> > &resg, Vector<Vector<gentype> > &gvarres, int minbad, int maxbad, double noisemean = 0, double noisevar = 0, int startpoint = 0, int calcvarres = 0, int suppressfb = 0, int useThreads = 0);
