...
def run_main(ipath,bo,opath,labels_total,thick,angles,gap_list,zdim):
    print(thick)
    return thick[0]+thick[1]
...
def runner(h):
    ...
    ipath="/vast/dylana/BO-Muse-master-update/src/cylinder_plate.k"
    bo = "/vast/dylana/BO-Muse-master-update/src/"
    opath="/vast/dylana/BO-Muse-master-update/src"

    #--------------------------------------------------
    #read in the max values used for normalising. This provided as a text file which is generated with the feasible design plots.
    min_max_df = pd.read_csv('/vast/dylana/BO-Muse-master/python/min_max_values.txt', sep='\t', index_col=0)
...
def runner_(h):
    ...
    #specify the maximum and minium allowable design variables:
    angle_range = [-60,60]
    thick_range = [5, 100]
    gap_range   = [0,100]
    plate_length = 0.2
    ...
    thick1=h[0]*(thick_range[1]-thick_range[0]) + thick_range[0]
    thick2=h[2]*(thick_range[1]-thick_range[0]) + thick_range[0]
    thick=[thick1,thick2]
    ...
    plate2=calculate_plate_location(plate1, plate2, angle1rad, gap1, thick1, thick2,gap2)  #this is needed for every case that isnt
    ...
    #may still need this if we are still generating a grid of data
    #min_max_df = pd.read_csv('\home\dylana\github\BO-Muse\python\min_max_values.txt', sep='\t', index_col=0)
    min_max_df = pd.read_csv('min_max_values.txt', sep='\t', index_col=0)
    mins = min_max_df['Min'].to_numpy() 
    maxs = min_max_df['Max'].to_numpy()      
    stren1=h[6]*(maxs[6]-mins[6]) + mins[6]   
    stren2=h[9]*(maxs[9]-mins[9]) + mins[9]   
    ...
    return (tw_norm, 0, [pval])




def calcAD(h):
    "Calculate the AD only"

    #Case where the prior is called for flagging purposes
    if isinstance(h, type(None)):
        return 0

    #Case where the storage block attempts to calculate priors on grid data (without filling continuous variables)
    if None in h:
        return 0
    ...
    #maxs = min_max_df['Max'].to_numpy()
    #min_max_df = pd.read_csv('min_max_values.txt', sep='\t', index_col=0)
    #mins_py = min_max_df['Min']
    #maxs_py = min_max_df['Max']
    #mins = mins_py.to_numpy()
    #maxs = maxs_py.to_numpy()
    mins_py = [5.000000e+00,-6.000000e+01,5.000000e+00,-6.000000e+01,0.000000e+00,2.700000e+03,3.930000e+08,7.000000e-02,2.700000e+03,3.930000e+08,7.000000e-02,7.840000e+01]
    maxs_py = [8.000000e+01,6.000000e+01,8.000000e+01,6.000000e+01,1.000000e+02,7.850000e+03,1.450000e+09,1.400000e-01,7.850000e+03,1.450000e+09,1.400000e-01,8.640000e+02]
    mins = np.array(mins_py)
    maxs = np.array(maxs_py)
    ...
    return float(tw_norm/2)



# Example usage
if __name__ == "__main__":
    h=[ 0.52458710179175383	,  0.26330554078427826	,  0.19628582558072902	,  0.51231810869672811	,  1,  1,  0.14285714285714277	,  0,  0	,  0	, 0	  ]

    #density1=0.9980582524271845*(maxs[6]-mins[6]) + mins[6]   
#density2=0.9980582524271845*(maxs[9]-mins[9]) + mins[9] 
    runner_(h)
