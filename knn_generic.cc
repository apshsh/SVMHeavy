...
KNN_Generic::KNN_Generic() : ML_Base()
{
    ...
    MEMNEW(Gpdist,Matrix<double>(Kcache_celm_v_double,Kcache_celm_double,Kcache_crow_double,(void *) &kerncache ,0,0));
...
void KNN_Generic::dgTrainingVector(Vector<gentype> &res, gentype &resn, int p) const
{
    ...
    if ( N() )
    {
        effkay = distcalcTrainingVector(effkay,iiscr,kdistscr,p,Nnz);
        ...
        const Vector<gentype> &yk = y()(iiscr(0,1,effkay-1,tmpvd),tmpve);
        const Vector<double> &kdistk = kdistscr(iiscr(0,1,effkay-1,tmpvf),tmpvg);
        ...
        if ( effkay )
        {
            ...
            Vector<gentype> &xres = res("&",iiscr(0,1,effkay-1,tmpva),tmpvb);
...
int KNN_Generic::getparam(int ind, gentype &val, const gentype &xa, int ia, const gentype &xb, int ib, charptr &desc) const
{
    int res = 0;
    ...
    switch ( ind )
    {
        case 4000: { val = k();   desc = "KNN_Generic::k"; break; }
        case 4001: { val = ktp(); desc = "KNN_Generic::ktp"; break; }
