...
// -------------------------------------------------------------------------
    ...
    mult(xx("&",xaN+xbN,1,xaN+xbN+xaN+xxN-1,tmpva),AA(0,1,xaN+xxN-1,0,1,xaN+xbN-1,tmpma),xx(0,1,xaN+xbN-1,tmpvb));
    xx("&",xaN+xbN,1,xaN+xbN+xaN+xxN-1,tmpva) += bb(0,1,xaN+xxN-1,tmpvb);
...
errstream() << "pre;;;";
        ...
        linsolvetrans(killSwitch,xx,constrtype,AA,xfalt,bb,sc); //maxitcntint,xmtrtime,xmtrtimeend);
    }
    ...
    linsolvetrans(killSwitch,xx,constrtype,AA,xf,bb,sc); //maxitcntint,xmtrtime,xmtrtimeend);
...
errstream() << "Core entry...";
    int res = linsolvebase(killSwitch,x,G,c,g,sc); //maxitcntint,xmtrtime,xmtrtimeend);
...
errstream() << "Result retrieval...";
    //if ( alphabchi.size() )
    {
        for ( int i = 0 ; i < alphabchi.size() ; ++i )
        {
...
//errstream() << "x = " << x << "\n\n\n\n";
            ...
            sc.testopt(itcnt,isopt,timeout,bailout,stopout,kickstart,FEEDBACK_CYCLE,MAJOR_FEEDBACK_CYCLE,start_time,curr_time,"linear optimisation",uservars,varnames,vardescr);
	}
