...
#ifndef _lsv_mvrank_h
#define _lsv_mvrank_h
...
// Implementation: just a glass-typewriter over SVM_MvRank with appropriate
// setting mods
...
inline void qswap(LSV_MvRank &a, LSV_MvRank &b);
inline LSV_MvRank &setzero(LSV_MvRank &a);

class LSV_MvRank : public LSV_Generic
{
public:
    ...
    LSV_MvRank();
    LSV_MvRank(const LSV_MvRank &src);
    LSV_MvRank(const LSV_MvRank &src, const ML_Base *srcx);
    LSV_MvRank &operator=(const LSV_MvRank &src) { assign(src); return *this; }
    virtual ~LSV_MvRank() { return; }
    ...
    virtual int type   (void)  const override { return 510;          }
    ...
    virtual int addTrainingVector (int i, double z, const SparseVector<gentype> &x, double Cweigh = 1, double epsweigh = 1, int d = 2) override { return getQ().addTrainingVector (i,z,x,Cweigh,epsweigh,d); }
    virtual int qaddTrainingVector(int i, double z,       SparseVector<gentype> &x, double Cweigh = 1, double epsweigh = 1, int d = 2) override { return getQ().qaddTrainingVector(i,z,x,Cweigh,epsweigh,d); }
    ...
    virtual int setZerodelta(void) override { return getQ().setFixedBias(0.0); }
    ...
    virtual       SVM_MvRank &getQ(void)            { return grunt; }
    virtual const SVM_MvRank &getQconst(void) const { return grunt; }
...
inline double norm2(const LSV_MvRank &a);
inline double abs2 (const LSV_MvRank &a);

inline double norm2(const LSV_MvRank &a) { return a.RKHSnorm(); }
inline double abs2 (const LSV_MvRank &a) { return a.RKHSabs();  }
...
inline void LSV_MvRank::qswapinternal(ML_Base &bb)
{
    NiceAssert( isQswapCompat(*this,bb) );

    LSV_MvRank &b = dynamic_cast<LSV_MvRank &>(bb.getML());
...
inline void LSV_MvRank::semicopy(const ML_Base &bb)
{
    NiceAssert( isSemicopyCompat(*this,bb) );

    const LSV_MvRank &b = dynamic_cast<const LSV_MvRank &>(bb.getMLconst());
...
inline void LSV_MvRank::assign(const ML_Base &bb, int onlySemiCopy)
{
    NiceAssert( isAssignCompat(*this,bb) );

    const LSV_MvRank &src = dynamic_cast<const LSV_MvRank &>(bb.getMLconst());
