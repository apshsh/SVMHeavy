...
public:
    ...
    virtual int sety(int                i,       double          y);
    ...
    virtual double eTrainingVector(int i) const;

    virtual int covTrainingVector(gentype &resv, gentype &resmu, int i, int j, gentype ***pxyprodi = nullptr, gentype ***pxyprodj = nullptr, gentype **pxyprodij = nullptr) const;

    virtual double         &dedgTrainingVector(double         &res, int i) const;
    virtual Vector<double> &dedgTrainingVector(Vector<double> &res, int i) const { dedgTrainingVector((res.resize(1))("&",0),i);   return res; }
    virtual d_anion        &dedgTrainingVector(d_anion        &res, int i) const { dedgTrainingVector((res.setorder(0))("&",0),i); return res; }
    virtual gentype        &dedgTrainingVector(gentype        &res, int i) const { dedgTrainingVector(res.force_double(),i);       return res;  }

    virtual double &d2edg2TrainingVector(double &res, int i) const;

    virtual double dedKTrainingVector(int i, int j) const { double tmp; return dedgTrainingVector(tmp,i)*alphaR()(j); }
    virtual Vector<double> &dedKTrainingVector(Vector<double> &res, int i) const;
    virtual Matrix<double> &dedKTrainingVector(Matrix<double> &res) const;
    ...
    virtual       double          biasR (void) const { return dbiasR;  }
