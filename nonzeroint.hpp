...
public:
    ...
    explicit nzint()                 : val(-1)      { }
    explicit nzint(int src)          : val(src)     { }
             nzint(const nzint &src) : val(src.val) { }
...
inline nzint &setrand  (nzint &a          ) { a = (2*(rand()%2))-1;        return a; } //a = (2*(svm_rand()%2))-1;        return a; }
