...
ONN_Generic::ONN_Generic() : ML_Base()
{
    ...
    dmtrtimeend = NN_DEFAULT_TRAINTIMEEND;
    dlr         = NN_DEFAULT_LR;
...
std::ostream &ONN_Generic::printstream(std::ostream &output, int dep) const
{
    ...
    repPrint(output,'>',dep) << "ONN end training time:      " << dmtrtimeend << "\n";
...
//    int dummy;
    ...
    if ( N() )
    {
        ...
        double xot = Opttol();
        double *uservars[] = { &maxitcnt, &xmtrtime, &xmtrtimeend, &xlr, &xot, nullptr };
        const char *varnames[] = { "itercount", "traintime", "trainendtime", "learning rate", "optimisation tolerance", nullptr };
        const char *vardescr[] = { "Maximum iteration count (0 for unlimited)", "Maximum training time (seconds, 0 for unlimited)", "Absolute training end time (seconds, relative, -1 for n/a)", "Learning rate", "Optimality tolerance", nullptr };
...
int ONN_Generic::getparam(int ind, gentype &val, const gentype &xa, int ia, const gentype &xb, int ib, charptr &desc) const
{
    int res = 0;
    ...
    switch ( ind )
    {
        case 7000: { val = lr();                  desc = "ONN_Generic::lr"; break; }
        case 7001: { convertSparseToSet(val,W()); desc = "ONN_Generic::convertSparseToSet"; break; }
        case 7004: { val = B();                   desc = "ONN_Generic::B"; break; }
