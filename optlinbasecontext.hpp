...
public:
    ...
    void refact(const Matrix<double> &H, double xzt = -1) { Q.refact(H,Gn,Gpn,1,xzt); }
    void reset (const Matrix<double> &H)                  { Q.reset(H,Gn,Gpn);        }
    ...
    void fudgeOn (const Matrix<double> &H) { Q.fact_fudgeOn(H,Gn,Gpn,apos,bpos);  }
    void fudgeOff(const Matrix<double> &H) { Q.fact_fudgeOff(H,Gn,Gpn,apos,bpos); }
