...
void optLinContext::refact(const Matrix<double> &G, double xzt)
{
    ...
    //if ( rowN() )
    {
        ...
        for ( int i = 0 ; i < rowN() ; ++i )
        {
            ...
            {
                innerProduct(H("&",i,j),G(i,pivAlphaF(),tmpva,tmpvb,tmpve),G(j,pivAlphaF(),tmpvc,tmpvd,tmpve));
            }
        }
    }
...
void optLinContext::reset(const Matrix<double> &)
{
    //(void) G;
...
int optLinContext::addRow(int i)
{
    ...
    {
        for ( int iP = 0 ; ( iP < Hblanks.size() ) && ucnt ; ++iP )
        {
            if ( Hblanks(iP) >= i )
            {
                ++(Hblanks("&",iP));
...
void optLinContext::fixH(const Matrix<double> &G)
{
    if ( Hblanks.size() )
    {
        ...
        for ( i = Hblanks.size()-1 ; i >= 0 ; --i )
        {
            for ( j = 0 ; j < rowN() ; ++j )
            {
                innerProduct(H("&",i,j),G(i,pivAlphaF(),tmpva,tmpvb,tmpve),G(j,pivAlphaF(),tmpvc,tmpvd,tmpvf));
                H("&",j,i) = H(i,j);
            }
