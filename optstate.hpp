...
public:
    ...
    //
    // lastN: 0 means do as described above
    //        1 means apply to last NRff vectors, constrain the rest to zero, constrain beta

    int modAllToDesthpzero(const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, bool lastN = false, int NRff = 0);
    ...
    int pivAlphaLB(int iP) const { return probContext.pivAlphaLB(iP); }
    int pivAlphaZ (int iP) const { return probContext.pivAlphaZ(iP);  }
    int pivAlphaUB(int iP) const { return probContext.pivAlphaUB(iP); }
    int pivAlphaF (int iP) const { return probContext.pivAlphaF(iP);  }

    int pivBetaC(int iP) const { return probContext.pivBetaC(iP); }
    int pivBetaF(int iP) const { return probContext.pivBetaF(iP); }

    int alphaState(int i) const { return probContext.alphaState(i); }
    int betaState (int i) const { return probContext.betaState(i);  }
    ...
    const T alpha_v(int i) const { return dalpha.v(i); }
    const T beta_v (int i) const { return dbeta.v(i);  }

    const T &alphaGrad(int i) const { NiceAssert( !gradFixAlpha.v(i) ); return dalphaGrad(i); }
    const T &betaGrad (int i) const { NiceAssert( !gradFixBeta.v(i) );  return dbetaGrad(i);  }

    const T alphaGrad_v(int i) const { NiceAssert( !gradFixAlpha.v(i) ); return dalphaGrad.v(i); }
    const T betaGrad_V (int i) const { NiceAssert( !gradFixBeta.v(i) );  return dbetaGrad.v(i);  }
    ...
    //                  dalphaGrad(pivAlphaF(0,1,asize-1)), betaGrad(pivBetaF()(0,1,bsize-1))
    ...
    template <class U>
    int fact_minverse(Vector<U> &aAlpha, Vector<U> &aBeta, const Vector<U> &bAlpha, const Vector<U> &bBeta, const Matrix<double> &Gp, const Matrix<double> &Gn, const Matrix<double> &Gpn) const;
    double fact_det   (void) const { return probContext.fact_det();    }
    double fact_logdet(void) const { return probContext.fact_logdet(); }
    template <class U>
    void fact_minvdiagsq(Vector<U> &ares, Vector<U> &bres, const Matrix<double> &Gn, const Matrix<double> &Gpn) const;
...
template <class T, class S>
void optState<T,S>::setAlphaF(const Vector<T> &newAlpha, const Matrix<double> &, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp, const Vector<double> &lb, const Vector<double> &ub, double ztoloverride)
{
    NiceAssert( newAlpha.size() == aN() );
...
void optState<T,S>::setBetahpzero(const Vector<T> &newBeta, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, double ztoloverride)
{
	...
	for ( i = 0 ; i < bN() ; ++i )
	{
	    ...
	    if ( newBeta(i) >= ztoloverride )
	    {
                ...
                betaStephpzero(i,newBeta(i)-beta(i),GpGrad,Gn,Gpn,gp,gn);
	    }
...
void optState<T,S>::refactGpnElm(const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &GpnOld, double GpnijNew, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp, int i, int j)
{
    ...
    dalphaGrad("&",i) -= (GpnOld(i,j)*beta(j));
    dalphaGrad("&",i) += (GpnijNew*beta(j));
...
template <class T, class S>
int optState<T,S>::fact_snookered(const Matrix<double> &, const Matrix<double> &, const Matrix<double> &)
{
    int res = 0;
...
template <class T, class S>
void optState<T,S>::rankone(const Vector<double> &bp, const Vector<double> &bn, double c, const Vector<double> &bpGrad, const Vector<double> &bnGrad, double cGrad, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp)
{
    //comment out to enable threading in errortest NiceAssert( Gp.isSquare() );
    //comment out to enable threading in errortest NiceAssert( GpGrad.isSquare() );
    NiceAssert( Gn.isSquare() );
	...
	T inner;
        ...
        inner  = twoProduct(temp,bpGrad(pivAlphaLB()),dalpha(pivAlphaLB()));
        inner += twoProduct(temp,bpGrad(pivAlphaF ()),dalpha(pivAlphaF ()));
        inner += twoProduct(temp,bpGrad(pivAlphaUB()),dalpha(pivAlphaUB()));
        inner += twoProduct(temp,bnGrad(pivBetaF  ()),dbeta (pivBetaF  ()));
        inner *= cGrad;
...
void optState<T,S>::diagoffset(int i, double bpoff, double bpoffGrad, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp)
{
    ...
    if ( alphaState(i) != 0 )
    {
	dalphaGrad("&",i) += bpoffGrad*alpha(i);
...
void optState<T,S>::changeAlphaRestrict(int i, int alphrestrict, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp)
{
    //comment out to enable threading in errortest NiceAssert( Gp.isSquare() );
    NiceAssert( Gn.isSquare() );
    //comment out to enable threading in errortestNiceAssert( Gp.numRows() == Gpn.numRows() );
    NiceAssert( Gn.numCols() == Gpn.numCols() );
    NiceAssert( Gpn.numRows() == aN() );
    NiceAssert( Gpn.numCols() == bN() );
    NiceAssert( gp.size() == aN() );
...
//		iP = modAlphaLBtoLF(iP,Gp,GpGrad,Gn,Gpn,gp,gn,hp);
//		alphaStep(i,-alpha(i),GpGrad,Gn,Gpn,gp,gn,hp);
//		iP = modAlphaLFtoZ(iP,Gp,GpGrad,Gn,Gpn,gp,gn,hp);
		alphaStep(i,-alpha(i),GpGrad,Gn,Gpn,gp,gn,hp,1); // Note dontCheckState use here
		iP = modAlphaLBtoZ(iP,Gp,GpGrad,Gn,Gpn,gp,gn,hp);
	    }
...
void optState<T,S>::changeBetaRestrict (int i, int betrestrict, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp)
{
	...
	{
	    betaStep(i,-beta(i),GpGrad,Gn,Gpn,gp,gn,hp);
	}

	iP = modBetaFtoC(iP,Gp,GpGrad,Gn,Gpn,gp,gn,hp);
    }

    dbetaRestrict.sv(i,betrestrict);
}
...
//FIXME: this can be done without disturbing the active set
//                iP = modAlphaLBtoLFhpzero(iP,Gp,GpGrad,Gn,Gpn,gp,gn);
//                alphaStephpzero(i,-alpha(i),GpGrad,Gn,Gpn,gp,gn);
//                iP = modAlphaLFtoZhpzero(iP,Gp,GpGrad,Gn,Gpn,gp,gn);
                alphaStephpzero(i,-alpha(i),GpGrad,Gn,Gpn,gp,gn,1); // Note dontCheckState use here
                iP = modAlphaLBtoZhpzero(iP,Gp,GpGrad,Gn,Gpn,gp,gn);
	    }
...
void optState<T,S>::changeBetaRestricthpzero (int i, int betrestrict, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn)
{
	...
	if ( ( ( betaState(i) < 0 ) && ( ( betrestrict == 1 ) || ( betrestrict == 3 ) ) ) || ( ( betaState(i) > 0 ) && ( ( betrestrict == 2 ) || ( betrestrict == 3 ) ) ) )
	{
            betaStephpzero(i,-beta(i),GpGrad,Gn,Gpn,gp,gn);
	}
...
//    NiceAssert( isposcond(alpha(pivAlphaF(res)),-dopttol) );
    ...
    if ( ( abs2(GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))) > STEPTRIGGER*dopttol ) && ( isneg(alpha(pivAlphaF(res))) ) )
    {
	alphaStephpzero(pivAlphaF(res),-alpha(pivAlphaF(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }

    if ( isneg(alpha(pivAlphaF(res))) )
    {
        cumgraderr += abs2((GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))));
...
//    NiceAssert( isnegcond(alpha(pivAlphaF(res)),dopttol) );
    ...
    if ( ( abs2(GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))) > STEPTRIGGER*dopttol ) && ( ispos(alpha(pivAlphaF(res))) ) )
    {
	alphaStephpzero(pivAlphaF(res),-alpha(pivAlphaF(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }

    if ( ispos(alpha(pivAlphaF(res))) )
    {
        cumgraderr += abs2((GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))));
...
{ 
    ...
    if ( abs2(GpGrad(pivAlphaLB(res),pivAlphaLB(res))*(lb.v(pivAlphaLB(res))-alpha(pivAlphaLB(res)))) > STEPTRIGGER*dopttol )
    {
	alphaStephpzero(pivAlphaLB(res),lb.v(pivAlphaLB(res))-alpha(pivAlphaLB(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }

    cumgraderr += abs2((GpGrad(pivAlphaLB(res),pivAlphaLB(res))*(lb.v(pivAlphaLB(res))-alpha(pivAlphaLB(res)))));

    dalpha("&",pivAlphaLB(res)) = lb.v(pivAlphaLB(res));
...
{ 
    ...
    if ( abs2(GpGrad(pivAlphaZ(res),pivAlphaZ(res))*alpha(pivAlphaZ(res))) > STEPTRIGGER*dopttol )
    {
	alphaStephpzero(pivAlphaZ(res),-alpha(pivAlphaZ(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }

    cumgraderr += abs2((GpGrad(pivAlphaZ(res),pivAlphaZ(res))*alpha(pivAlphaZ(res))));
...
{ 
    ...
    if ( abs2(GpGrad(pivAlphaUB(res),pivAlphaUB(res))*(ub.v(pivAlphaUB(res))-alpha(pivAlphaUB(res)))) > STEPTRIGGER*dopttol )
    {
	alphaStephpzero(pivAlphaUB(res),ub.v(pivAlphaUB(res))-alpha(pivAlphaUB(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }

    cumgraderr += abs2((GpGrad(pivAlphaUB(res),pivAlphaUB(res))*(ub.v(pivAlphaUB(res))-alpha(pivAlphaUB(res)))));

    dalpha("&",pivAlphaUB(res)) = ub.v(pivAlphaUB(res));
...
{ 
    ...
    NiceAssert( iP >= 0 );
    NiceAssert( iP < aNZ() );
    NiceAssert( ( dalphaRestrict.v(pivAlphaZ(iP)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(iP)) == 2 ) );
...
{ 
    ...
    NiceAssert( iP >= 0 );
    NiceAssert( iP < aNZ() );
    NiceAssert( ( dalphaRestrict.v(pivAlphaZ(iP)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(iP)) == 1 ) );
...
template <class T, class S>
int optState<T,S>::modAllToDesthpzero(const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, bool lastN, int NRff)
{
    // Free/constrain all alphas and betas based on alphaRestrict and
    // betaRestrict (free if restrict == 0, constrained if restrict == 3,
    // throw otherwise).  Will also change (or set) alpha UF or LF depending
    ...
    // Step 4: mod {L,U}F to Z for all Z constrained variabled with !astat (if lastN then also do this for all except the last NRff variables)
    ...
    if ( aNF() )
    {
        ...
        while ( !clearrun )
        {
            ...
            for ( iP = aNF()-1 ; iP >= 0 ; --iP )
            {
                ...
                if ( ( ( alphaRestrict()(i) == 3 ) || ( lastN && ( i < aN()-NRff ) ) ) && ( alphaState(i) == +1 ) )
                {
                    alphaStephpzero(i,-alpha(i),GpGrad,Gn,Gpn,gp,gn);
                ...
                else if ( ( ( alphaRestrict()(i) == 3 ) || ( lastN && ( i < aN()-NRff ) ) ) && ( alphaState(i) == -1 ) )
                {
                    alphaStephpzero(i,-alpha(i),GpGrad,Gn,Gpn,gp,gn);
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    if ( ( abs2(GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))) > STEPTRIGGER*dopttol ) && ( alpha(pivAlphaF(res)) < 0 ) )
    {
	alphaStephpzero(pivAlphaF(res),-alpha(pivAlphaF(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }

    if ( alpha(pivAlphaF(res)) < 0 )
    {
        cumgraderr += abs2((GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))));
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    if ( ( abs2(GpGrad(pivAlphaF(res),pivAlphaF(res))*alpha(pivAlphaF(res))) > STEPTRIGGER*dopttol ) && ( alpha(pivAlphaF(res)) > 0 ) )
    {
	alphaStep(pivAlphaF(res),-alpha(pivAlphaF(res)),GpGrad,Gn,Gpn,gp,gn,hp,1);
    }
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    if ( abs2(GpGrad(pivAlphaLB(res),pivAlphaLB(res))*(lb.v(pivAlphaLB(res))-alpha(pivAlphaLB(res)))) > STEPTRIGGER*dopttol )
    {
	alphaStep(pivAlphaLB(res),lb.v(pivAlphaLB(res))-alpha(pivAlphaLB(res)),GpGrad,Gn,Gpn,gp,gn,hp,1);
    }
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    if ( abs2(GpGrad(pivAlphaZ(res),pivAlphaZ(res))*alpha(pivAlphaZ(res))) > STEPTRIGGER*dopttol )
    {
	alphaStep(pivAlphaZ(res),-alpha(pivAlphaZ(res)),GpGrad,Gn,Gpn,gp,gn,hp,1);
    }
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    if ( abs2(GpGrad(pivAlphaUB(res),pivAlphaUB(res))*(ub.v(pivAlphaUB(res))-alpha(pivAlphaUB(res)))) > STEPTRIGGER*dopttol )
    {
	alphaStep(pivAlphaUB(res),ub.v(pivAlphaUB(res))-alpha(pivAlphaUB(res)),GpGrad,Gn,Gpn,gp,gn,hp,1);
    }
...
int optState<T,S>::modBetaCtoF(int iP, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp)
{
    ...
    NiceAssert( iP >= 0 );
    NiceAssert( iP < bNC() );
    NiceAssert( dbetaRestrict.v(pivBetaC(iP)) != 3 );
...
int optState<T,S>::modBetaFtoC(int iP, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn, const Vector<T> &hp)
{
    ...
    if ( Gn.v(pivBetaC(res),pivBetaC(res))*abs2(beta(pivBetaC(res))) > STEPTRIGGER*dopttol )
    {
	betaStep(pivBetaC(res),-beta(pivBetaC(res)),GpGrad,Gn,Gpn,gp,gn,hp,1);
    }

    cumgraderr += (Gn.v(pivBetaC(res),pivBetaC(res))*abs2(beta(pivBetaC(res))));
...
int optState<T,S>::modBetaFtoChpzero(int iP, const Matrix<double> &Gp, const Matrix<S> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<T> &gp, const Vector<T> &gn)
{
    ...
    if ( Gn.v(pivBetaC(res),pivBetaC(res))*abs2(beta(pivBetaC(res))) > STEPTRIGGER*dopttol )
    {
	betaStephpzero(pivBetaC(res),-beta(pivBetaC(res)),GpGrad,Gn,Gpn,gp,gn,1);
    }
...
template <> inline int optState<double,double>::scaleFStepbase(double &scale, int &alphaFIndex, int &betaFIndex, int &betaCIndex, int &stateChange, int asize, int bsize, int &bailout, Vector<double> &alphaFStep, Vector<double> &betaFStep, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<double> &lb, const Vector<double> &ub, int hpzero);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	for ( i = 0 ; ( i < asize ) && !bailout ; ++i )
	{
		...
		{
		    if ( alpha(pivAlphaF(i))+(scale*alphaFStep(i)) >= zero_overrun )
		    {
			potscale = -alpha(pivAlphaF(i))/alphaFStep(i);
...
//errstream() << "WARNING: scaling outside bounds (type 1) " << potscale << "\n";
		...
		{
		    if ( alpha(pivAlphaF(i))+(scale*alphaFStep(i)) >= ub.v(pivAlphaF(i))+zero_overrun )
		    {
			potscale = (ub.v(pivAlphaF(i))-alpha(pivAlphaF(i)))/alphaFStep(i);
...
//errstream() << "WARNING: scaling outside bounds (type 2) " << potscale << "\n";
		...
		{
		    if ( alpha(pivAlphaF(i))+(scale*alphaFStep(i)) <= lb.v(pivAlphaF(i))-zero_overrun )
		    {
			potscale = (lb.v(pivAlphaF(i))-alpha(pivAlphaF(i)))/alphaFStep(i);
...
//errstream() << "WARNING: scaling outside bounds (type 6) " << potscale << "\n";
	...
	for ( i = 0 ; ( i < bsize ) && !bailout ; ++i )
	{
	    ...
	    {
		if ( beta(pivBetaF(i))+(scale*betaFStep(i)) >= zero_overrun )
		{
		    potscale = -beta(pivBetaF(i))/betaFStep(i);
...
//errstream() << "WARNING: scaling outside bounds (type 7) " << potscale << "\n";
	    ...
	    else if ( ( dbetaRestrict.v(pivBetaF(i)) == 1 ) && ( scale*betaFStep(i) < 0 ) )
	    {
		if ( beta(pivBetaF(i))+(scale*betaFStep(i)) <= -zero_overrun )
		{
		    potscale = -beta(pivBetaF(i))/betaFStep(i);
...
errstream() << "&&";
	    ...
	    else if ( ( dbetaRestrict.v(pivBetaC(iP)) == 2 ) && ( betaGrad(pivBetaC(iP))+betaGradStepC(iP) < 0 ) )
	    {
		potscale = -1/zerotol(); // very big number
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	for ( i = 0 ; ( i < asize ) && !bailout ; ++i )
	{
	    ...
	    {
		if ( ( alphaState(pivAlphaF(i)) == -1 ) && ( scale*alphaFStep.v(i) > 0 ) && ( alphaRestrict(pivAlphaF(i)) || !hpzero ) )
		{
		    if ( dalpha.v(pivAlphaF(i))+(scale*alphaFStep.v(i)) >= zero_overrun )
		    {
			potscale = -dalpha.v(pivAlphaF(i))/alphaFStep.v(i);
...
//errstream() << "WARNING: scaling outside bounds (type 1) " << potscale << "\n";
		...
		else if ( ( alphaState(pivAlphaF(i)) == -1 ) && ( scale*alphaFStep.v(i) > 0 ) && ( !alphaRestrict(pivAlphaF(i)) && hpzero ) )
		{
		    if ( dalpha.v(pivAlphaF(i))+(scale*alphaFStep.v(i)) >= ub.v(pivAlphaF(i))+zero_overrun )
		    {
			potscale = (ub.v(pivAlphaF(i))-dalpha.v(pivAlphaF(i)))/alphaFStep.v(i);
...
//errstream() << "WARNING: scaling outside bounds (type 2) " << potscale << "\n";
		...
		else if ( ( alphaState(pivAlphaF(i)) == -1 ) && ( scale*alphaFStep.v(i) < 0 ) )
		{
		    if ( dalpha.v(pivAlphaF(i))+(scale*alphaFStep.v(i)) <= lb.v(pivAlphaF(i))-zero_overrun )
		    {
			potscale = (lb.v(pivAlphaF(i))-dalpha.v(pivAlphaF(i)))/alphaFStep.v(i);
...
//errstream() << "WARNING: scaling outside bounds (type 6) " << potscale << "\n";
	...
	for ( i = 0 ; ( i < bsize ) && !bailout ; ++i )
	{
	    ...
	    {
		if ( dbeta.v(pivBetaF(i))+(scale*betaFStep.v(i)) >= zero_overrun )
		{
		    potscale = -dbeta.v(pivBetaF(i))/betaFStep.v(i);
...
//errstream() << "WARNING: scaling outside bounds (type 7) " << potscale << "\n";
	    ...
	    else if ( ( dbetaRestrict.v(pivBetaF(i)) == 1 ) && ( scale*betaFStep.v(i) < 0 ) )
	    {
		if ( dbeta.v(pivBetaF(i))+(scale*betaFStep.v(i)) <= -zero_overrun )
		{
		    potscale = -dbeta.v(pivBetaF(i))/betaFStep.v(i);
...
//errstream() << "WARNING: scaling outside bounds (type 8) " << potscale << "\n";
	...
	for ( iP = 0 ; ( iP < bNC() ) && !bailout ; ++iP )
	{
		...
		potscale = -1/zerotol(); // very big number
                ...
                if ( abs2(betaGradStepC.v(iP)) >= zerotol() )
		{
...
errstream() << "&&";
	    ...
	    else if ( ( dbetaRestrict.v(pivBetaC(iP)) == 2 ) && ( dbetaGrad.v(pivBetaC(iP))+betaGradStepC.v(iP) < 0 ) )
	    {
		potscale = -1/zerotol(); // very big number
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    retVector<S>      tmpvd;
                ...
                // alphaStepbase is often called by setAlpha.  As a general rule, so as
                // to avoid sudden kcache flushes, we want to actively avoid touching the
                // most of Gp as
                // it is *very* large and mostly irrelevant (for RFF only a small block
                // on the bottom left, reflected at top right, as actually used).  We do
                // *not* want to touch an entire row during these calculations lest we
                // trigger an unneeded full-row calculation (particularly in the upper
                // part of Gp), which will push out other (more useful) rows.  Instead,
                // we use the slow variant of the alpha step function (alphaStepSlow),
                // which does all Gp access elementwise, which can be more easily
                // aliased using symmetry.

    if ( ( alphaState(i) == -1 ) || ( alphaState(i) == +1 ) )
    {
        // Likely kcache hit, so use the quick version

        dalphaGrad.scaleAddBR(GpGrad(i,0,1,aN()-1,tmpva,tmpvd),alphaStep);
    }

    else
    {
        // Likely kcache miss on entire row version, so use
...
void optState<T,S>::betaStepbase(int i, const T &betaStep , const Matrix<double> &Gn, const Matrix<double> &Gpn, int dontCheckState)
{
    ...
    dbetaGrad.scaleAddBR(Gn(i,tmpva,tmpvb),betaStep);
...
// See also specialisation below
template <> inline void optState<double,double>::stepFNewtonbase(double scale, int bsize, const Vector<double> &alphaFStep, const Vector<double> &betaFStep, const Matrix<double> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, int doCupdate, int doFupdate);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    dbeta ("&",pivBetaF (),0,1,bsize-1,tmpva) += betaFStep(0,1,bsize-1,tmpvb);
...
// See also specialisation below
template <> inline void optState<double,double>::stepFNewtonFullbase(int bsize, const Vector<double> &alphaFStep, const Vector<double> &betaFStep, const Matrix<double> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, int doCupdate, int doFupdate);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    dalpha("&",pivAlphaF(),tmpva)             += alphaFStep;
    dbeta ("&",pivBetaF (),0,1,bsize-1,tmpvb) += betaFStep(0,1,bsize-1,tmpvc);
...
// See also specialisation below
template <> inline void optState<double,double>::stepFLinearbase(int asize, int bsize, const Vector<double> &alphaFStep, const Vector<double> &betaFStep, const Matrix<double> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, int doCupdate, int doFupdate);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    (dalpha("&",pivAlphaF(),0,1,asize-1,tmpvb)) += alphaFStep(0,1,asize-1,tmpvc);
    (dbeta ("&",pivBetaF (),0,1,bsize-1,tmpvb)) += betaFStep(0,1,bsize-1,tmpvc);
	...
	if ( doCupdate )
	{
		...
		//are commented out and there is no change in alphagradstate or betagradstate.
                ...
                (dalphaGrad("&",pivAlphaF(),asize,1,aNF()-1,tmpvb)).scaleAddBR((GpGrad(pivAlphaF(iP),pivAlphaF(),tmpvc,tmpvd,tmpvf)(asize,1,aNF()-1,tmpve)),alphaFStep(iP));
                (dbetaGrad ("&",pivBetaF (),bsize,1,bNF()-1,tmpvb)).scaleAddBR((Gpn   (pivAlphaF(iP),pivBetaF (),tmpvc,tmpvd,tmpvf)(bsize,1,bNF()-1,tmpve)),alphaFStep(iP));
	...
	for ( iP = 0 ; iP < bsize ; ++iP )
	{
		...
		{
		    dalphaGrad("&",pivAlphaF(jP)) += (betaFStep(iP)*Gpn.v(pivAlphaF(jP),pivBetaF(iP)));
		}
	    }
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	if ( doCupdate )
	{
		...
		//are commented out and there is no change in alphagradstate or betagradstate.
                ...
                (dalphaGrad("&",pivAlphaF(),asize,1,aNF()-1,tmpvb)).scaleAddBR((GpGrad(pivAlphaF(iP),pivAlphaF(),tmpvc,tmpvd,tmpvf)(asize,1,aNF()-1,tmpve)),alphaFStep.v(iP));
                (dbetaGrad ("&",pivBetaF (),bsize,1,bNF()-1,tmpvb)).scaleAddBR((Gpn   (pivAlphaF(iP),pivBetaF (),tmpvc,tmpvd,tmpvf)(bsize,1,bNF()-1,tmpve)),alphaFStep.v(iP));

		//PRACTICE: in practice, thex2e must be a mistake somewhere in the above reasoning.
	...
	for ( iP = 0 ; iP < bsize ; ++iP )
	{
		...
		for ( jP = asize ; jP < aNF() ; ++jP )
		{
		    dalphaGrad("&",pivAlphaF(jP)) += (betaFStep.v(iP)*Gpn.v(pivAlphaF(jP),pivBetaF(iP)));
		}
	    }
...
// See also specialisation below
template <> inline void optState<double,double>::stepFGeneralbase(int asize, int bsize, const Vector<double> &alphaFStep, const Vector<double> &betaFStep, const Matrix<double> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, int doCupdate, int doFupdate);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	if ( doCupdate && doFupdate )
	{
	    for ( iP = 0 ; iP < asize ; ++iP )
	    {
                dalphaGrad.scaleAddBR(GpGrad(pivAlphaF(iP),0,1,aN()-1,tmpva,tmpvc),alphaFStep(iP));
                dbetaGrad.scaleAddBR(Gpn(pivAlphaF(iP),tmpva,tmpvc),alphaFStep(iP));
	    }
	}
	...
	else if ( !doCupdate && doFupdate )
	{
	    for ( iP = 0 ; iP < asize ; ++iP )
	    {
                dalphaGrad("&",pivAlphaF(),tmpva).scaleAddBR(GpGrad(pivAlphaF(iP),pivAlphaF(),tmpvb,tmpvc,tmpvd),alphaFStep(iP));
                dbetaGrad.scaleAddBR(Gpn(pivAlphaF(iP),tmpva,tmpvc),alphaFStep(iP));
	    }
	}
	...
	for ( iP = 0 ; iP < bsize ; ++iP )
	{
		...
		for ( jP = 0 ; jP < aNF() ; ++jP )
		{
            ...
            dbetaGrad.scaleAddBR(Gn(pivBetaF(iP),tmpva,tmpvd),betaFStep(iP));
	}
    }
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	if ( doCupdate && doFupdate )
	{
	    for ( iP = 0 ; iP < asize ; ++iP )
	    {
                dalphaGrad.scaleAddBR(GpGrad(pivAlphaF(iP),0,1,aN()-1,tmpva,tmpvc),alphaFStep.v(iP));
                dbetaGrad.scaleAddBR(Gpn(pivAlphaF(iP),tmpva,tmpvc),alphaFStep.v(iP));
	    }
	}
	...
	else if ( !doCupdate && doFupdate )
	{
	    for ( iP = 0 ; iP < asize ; ++iP )
	    {
                dalphaGrad("&",pivAlphaF(),tmpva).scaleAddBR(GpGrad(pivAlphaF(iP),pivAlphaF(),tmpvb,tmpvc,tmpvd),alphaFStep.v(iP));
                dbetaGrad.scaleAddBR(Gpn(pivAlphaF(iP),tmpva,tmpvc),alphaFStep.v(iP));
	    }
	}
	...
	for ( iP = 0 ; iP < bsize ; ++iP )
	{
		...
		for ( jP = 0 ; jP < aNF() ; ++jP )
		{
            ...
            dbetaGrad.scaleAddBR(Gn(pivBetaF(iP),tmpva,tmpvd),betaFStep.v(iP));
	}
    }
...
// See also specialisation below
template <> inline void optState<double,double>::updateGradOpt(const Vector<double> &combAlphaFStep, const Vector<double> &combBetaFStep, const Vector<double> &startAlphaGrad, const Vector<int> &FnF, const Vector<int> &startPivAlphaF, const Matrix<double> &GpGrad, const Matrix<double> &Gpn);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
    ...
    dalphaGrad.set(FnF,startAlphaGrad(FnF,tmpvb));
	...
	for ( iP = 0 ; iP < bNF() ; ++iP )
	{
		...
		for ( jP = 0 ; jP < aNLB() ; ++jP )
		{
		    dalphaGrad("&",pivAlphaLB(jP)) += (combBetaFStep(iP)*Gpn.v(pivAlphaLB(jP),pivBetaF(iP)));
		}
	    }
		...
		for ( jP = 0 ; jP < aNUB() ; ++jP )
		{
		    dalphaGrad("&",pivAlphaUB(jP)) += (combBetaFStep(iP)*Gpn.v(pivAlphaUB(jP),pivBetaF(iP)));
		}
	    }
	}
    }
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	for ( iP = 0 ; iP < bNF() ; ++iP )
	{
		...
		for ( jP = 0 ; jP < aNLB() ; ++jP )
		{
		    dalphaGrad("&",pivAlphaLB(jP)) += (combBetaFStep.v(iP)*Gpn.v(pivAlphaLB(jP),pivBetaF(iP)));
		}
	    }
		...
		for ( jP = 0 ; jP < aNUB() ; ++jP )
		{
		    dalphaGrad("&",pivAlphaUB(jP)) += (combBetaFStep.v(iP)*Gpn.v(pivAlphaUB(jP),pivBetaF(iP)));
		}
	    }
	}
    }
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	{
            if ( abs2(alphaGrad(i)-alphaGradTest(i)) > res )
	    {
                res = abs2(alphaGrad(i)-alphaGradTest(i));
		aerr = i;
                berr = -1;
	    }
	}
	...
	{
            if ( abs2(betaGrad(i)-betaGradTest(i)) > res )
	    {
                res = abs2(betaGrad(i)-betaGradTest(i));
		aerr = -1;
                berr = i;
	    }
	}
...
// See also specialisation below
template <> inline int optState<double,double>::maxGradNonOpt(int &alphaCIndex, int &betaCIndex, int &stateChange, double &gradmag, const Matrix<double> &GpGrad, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<double> &gp, const Vector<double> &gn, const Vector<double> &hp, double toloverride, int checkfree, int ignorebeta);
template <class T, class S>
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	{
	    if ( -alphaGrad(pivAlphaLB(i)) > gradmag )
	    {
		gradmag     = -alphaGrad(pivAlphaLB(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = -2;
	...
	{
	    if ( ( alphaGrad(pivAlphaZ(i))-hp(pivAlphaZ(i)) > gradmag ) && ( ( dalphaRestrict.v(pivAlphaZ(i)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(i)) == 2 ) ) )
	    {
                gradmag     = alphaGrad(pivAlphaZ(i))-hp(pivAlphaZ(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = -1;
	    ...
	    if ( ( -alphaGrad(pivAlphaZ(i))-hp(pivAlphaZ(i)) > gradmag ) && ( ( dalphaRestrict.v(pivAlphaZ(i)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(i)) == 1 ) ) )
	    {
                gradmag     = -alphaGrad(pivAlphaZ(i))-hp(pivAlphaZ(i));
		alphaCIndex = i;
		betaCIndex  = -1;
	...
	{
	    if ( alphaGrad(pivAlphaF(i))-hp(pivAlphaF(i)) > gradmag )
	    {
                gradmag     = alphaGrad(pivAlphaF(i))-hp(pivAlphaF(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = 0;
	...
	{
	    if ( alphaGrad(pivAlphaUB(i)) > gradmag )
	    {
                gradmag     = alphaGrad(pivAlphaUB(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = +2;
	...
	{
	    if ( ( -betaGrad(pivBetaC(i)) > gradmag ) && ( ( dbetaRestrict.v(pivBetaC(i)) == 0 ) || ( dbetaRestrict.v(pivBetaC(i)) == 2 ) ) )
	    {
                gradmag     = -betaGrad(pivBetaC(i));
		alphaCIndex = -1;
		betaCIndex  = i;
                stateChange = -1;
	    ...
	    if ( ( betaGrad(pivBetaC(i)) > gradmag ) && ( ( dbetaRestrict.v(pivBetaC(i)) == 0 ) || ( dbetaRestrict.v(pivBetaC(i)) == 1 ) ) )
	    {
                gradmag     = betaGrad(pivBetaC(i));
		alphaCIndex = -1;
		betaCIndex  = i;
	...
	{
            if ( abs2(betaGrad(pivBetaF(i))) > locgradmag )
	    {
                locgradmag = abs2(betaGrad(pivBetaF(i)));
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	for ( i = 0 ; i < pivAlphaZ().size() ; ++i )
	{
	    if ( ( dalphaGrad.v(pivAlphaZ(i))-hp.v(pivAlphaZ(i)) > gradmag ) && ( ( dalphaRestrict.v(pivAlphaZ(i)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(i)) == 2 ) ) )
	    {
                gradmag     = dalphaGrad.v(pivAlphaZ(i))-hp.v(pivAlphaZ(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = -1;
	    ...
	    if ( ( -dalphaGrad.v(pivAlphaZ(i))-hp.v(pivAlphaZ(i)) > gradmag ) && ( ( dalphaRestrict.v(pivAlphaZ(i)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(i)) == 1 ) ) )
	    {
                gradmag     = -dalphaGrad.v(pivAlphaZ(i))-hp.v(pivAlphaZ(i));
		alphaCIndex = i;
		betaCIndex  = -1;
	...
	for ( i = 0 ; i < pivAlphaF().size() ; ++i )
	{
	    if ( dalphaGrad.v(pivAlphaF(i))-hp.v(pivAlphaF(i)) > gradmag )
	    {
                gradmag     = dalphaGrad.v(pivAlphaF(i))-hp.v(pivAlphaF(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = 0;
	...
	for ( i = 0 ; i < pivBetaC().size() ; ++i )
	{
	    ...
	    {
                gradmag     = -dbetaGrad.v(pivBetaC(i));
		alphaCIndex = -1;
		betaCIndex  = i;
                stateChange = -1;
	    ...
	    if ( ( dbetaGrad.v(pivBetaC(i)) > gradmag ) && ( ( dbetaRestrict.v(pivBetaC(i)) == 0 ) || ( dbetaRestrict.v(pivBetaC(i)) == 1 ) ) )
	    {
                gradmag     = dbetaGrad.v(pivBetaC(i));
		alphaCIndex = -1;
		betaCIndex  = i;
	...
	for ( i = 0 ; i < pivBetaF().size() ; ++i )
	{
	    ...
	    {
                locgradmag = abs2(dbetaGrad.v(pivBetaF(i)));
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	for ( i = 0 ; i < pivAlphaZ().size() ; ++i )
	{
	    if ( ( alphaGrad(pivAlphaZ(i)) > gradmag ) && ( ( dalphaRestrict.v(pivAlphaZ(i)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(i)) == 2 ) ) )
	    {
                gradmag     = alphaGrad(pivAlphaZ(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = -1;
	    ...
	    if ( ( -alphaGrad(pivAlphaZ(i)) > gradmag ) && ( ( dalphaRestrict.v(pivAlphaZ(i)) == 0 ) || ( dalphaRestrict.v(pivAlphaZ(i)) == 1 ) ) )
	    {
                gradmag     = -alphaGrad(pivAlphaZ(i));
		alphaCIndex = i;
		betaCIndex  = -1;
	...
	for ( i = 0 ; i < pivAlphaF().size() ; ++i )
	{
	    if ( alphaGrad(pivAlphaF(i)) > gradmag )
	    {
                gradmag     = alphaGrad(pivAlphaF(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = 0;
...
//FIXME: if you want to include vector angles for hp then fix here
	...
	for ( i = 0 ; i < pivAlphaF().size() ; ++i )
	{
	    ...
	    {
                gradmag     = dalphaGrad.v(pivAlphaF(i));
		alphaCIndex = i;
		betaCIndex  = -1;
                stateChange = 0;
...
// See also specialisation below
template <> inline int optState<double,double>::fact_calcStepbase(Vector<double> &stepAlpha, Vector<double> &stepBeta, int &asize, int &bsize, const Matrix<double> &Gp, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<double> &lb, const Vector<double> &ub);
template <class T, class S>
int optState<T,S>::fact_calcStepbase(Vector<T> &stepAlpha, Vector<T> &stepBeta, int &asize, int &bsize, const Matrix<double> &Gp, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<double> &lb, const Vector<double> &ub)
{
	...
	stepBeta("&",pivBetaF(),bsize,1,bNF()-1,tmpvb).zero();
    }
	...
	double chng = 0;
        ...
        chng += real(twoProduct(temp,stepAlpha(pivAlphaF(),0,1,asize-1,tmpvb),dalphaGrad(pivAlphaF(),tmpvc)(0,1,asize-1,tmpvd)));
        chng -= real(twoProduct(temp,stepBeta (pivBetaF (),0,1,bsize-1,tmpvb),dbetaGrad (pivBetaF (),tmpvc)(0,1,bsize-1,tmpvd)));
	...
	if ( chng > 0.0 )
	{
	    ...
	    (stepAlpha("&",pivAlphaF(),0,1,asize  ,tmpvb)).negate();
            (stepBeta ("&",pivBetaF (),0,1,bsize-1,tmpvb)).negate();
	}
	...
	for ( iP = 0 ; iP < asize ; ++iP )
	{
	    ...
	    {
                distancetoedge = ub.v(pivAlphaF(iP)) - alpha(pivAlphaF(iP));
	    }

	    else if ( ( alphaState(pivAlphaF(iP)) > 0 ) && ( stepAlpha(pivAlphaF(iP)) <= -zerotol() ) && ( alphaRestrict(pivAlphaF(iP)) ) )
	    {
                distancetoedge = alpha(pivAlphaF(iP));
	    }

	    else if ( ( alphaState(pivAlphaF(iP)) > 0 ) && ( stepAlpha(pivAlphaF(iP)) <= -zerotol() ) && ( !alphaRestrict(pivAlphaF(iP)) ) )
	    {
                distancetoedge = alpha(pivAlphaF(iP)) - lb.v(pivAlphaF(iP));
	    }
	...
	stepAlpha("&",pivAlphaF(),0,1,probContext.fact_pfact(Gn,Gpn)  ,tmpvb) *= scale;
	stepBeta ("&",pivBetaF (),0,1,probContext.fact_nfact(Gn,Gpn)-1,tmpvb) *= scale;
        ...
        errstream() << "| bN = " << bNF() << "\n";
        errstream() << "| \n";
        errstream() << "| alpha step = " << stepAlpha(pivAlphaF(),tmpvaa) << "\n";
        errstream() << "| beta step = " << stepBeta(pivBetaF(),tmpvaa) << "\n";
        errstream() << "| \n";
        errstream() << "+----------------------------------\n";
        #endif
    }
    ...
    else
    {
        ...
        Chol<double> GpnGpnFact(GpnGpn(0,1,bsize-1,0,1,bsize-1,tmpmb),GnFake,GpnFake,onedoublevec(bsize,tmpvj),zerotol(),0);
        ...
        while ( ( bsize < bNF() ) && isok )
        {
            ...
            for ( i = bsize ; ( i < bNF() ) && !isok ; ++i )
            {
                ...
                if ( !(GpnGpnFact.add(bsize,1,GpnGpn(GpnInd,GpnInd,tmpmb,0,1,bsize,0,1,bsize),GnFake,GpnFake)) )
                {
                    ++bsize;
                    isok = 1;
                }
                ...
                else
                {
                    ...
                    GpnGpnFact.remove(bsize,GpnGpn(GpnInd,GpnInd,tmpmb,0,1,bsize-1,0,1,bsize-1),GnFake,GpnFake);
                }
            }
        }
        ...
        GpnGpnFact.minverse(g("&",GpnInd,0,1,bsize-1,tmpvf),andummy,p(GpnInd,0,1,bsize-1,tmpve),bndummy);
        ...
        twoProduct(dRleftaa,dalpha(pivAlphaF(),tmpvf),Gp(pivAlphaF(),pivAlphaF(),tmpma)*dalpha(pivAlphaF(),tmpvg));
        twoProduct(dRleftab,dalpha(pivAlphaF(),tmpvf)*Gpn(pivAlphaF(),pivBetaF(),tmpma),dbeta(pivBetaF(),tmpvg));
        twoProduct(dRleftaa,dbeta(pivBetaF(),tmpvf),Gn(pivBetaF(),pivBetaF(),tmpma)*dbeta(pivBetaF(),tmpvg));
        twoProduct(dRrighta,dalpha(pivAlphaF(),tmpvf),stepAlpha(pivAlphaF(),tmpvg));
        twoProduct(dRrightb,dbeta(pivBetaF(),tmpvf),stepBeta(pivBetaF(),tmpvg));
	...
	stepBeta ("&",pivBetaF (),tmpvf) *= scale;
        ...
        errstream() << "| bN = " << bNF() << "\n";
        errstream() << "| \n";
        errstream() << "| alpha step = " << stepAlpha(pivAlphaF(),tsmtv) << "\n";
        errstream() << "| beta step = " << stepBeta(pivBetaF(),tsmtv) << "\n";
        errstream() << "| \n";
        errstream() << "+----------------------------------\n";
        #endif
    }
...
template <> inline int optState<double,double>::fact_calcStepbase(Vector<double> &stepAlpha, Vector<double> &stepBeta, int &asize, int &bsize, const Matrix<double> &Gp, const Matrix<double> &Gn, const Matrix<double> &Gpn, const Vector<double> &lb, const Vector<double> &ub)
{
	...
	double chng = 0;
        ...
        chng += real(twoProduct(temp,stepAlpha(pivAlphaF(),0,1,asize-1,tmpvb),dalphaGrad(pivAlphaF(),0,1,asize-1,tmpvd)));
        chng -= real(twoProduct(temp,stepBeta (pivBetaF (),0,1,bsize-1,tmpvb),dbetaGrad (pivBetaF (),0,1,bsize-1,tmpvd)));

	if ( chng > 0.0 )
	{
	    (stepAlpha("&",pivAlphaF(),0,1,probContext.fact_pfact(Gn,Gpn)  ,tmpvb)).negate();
	...
	for ( iP = 0 ; iP < asize ; ++iP )
	{
	    if ( ( alphaState(pivAlphaF(iP)) > 0 ) && ( stepAlpha(pivAlphaF(iP)) >= zerotol() ) )
	    {
                distancetoedge = ub.v(pivAlphaF(iP)) - dalpha.v(pivAlphaF(iP));
	    }

	    else if ( ( alphaState(pivAlphaF(iP)) > 0 ) && ( stepAlpha.v(pivAlphaF(iP)) <= -zerotol() ) && ( alphaRestrict(pivAlphaF(iP)) ) )
	    {
                distancetoedge = dalpha.v(pivAlphaF(iP));
	    }

	    else if ( ( alphaState(pivAlphaF(iP)) > 0 ) && ( stepAlpha.v(pivAlphaF(iP)) <= -zerotol() ) && ( !alphaRestrict(pivAlphaF(iP)) ) )
	    {
                distancetoedge = dalpha.v(pivAlphaF(iP)) - lb.v(pivAlphaF(iP));
	    }
	    ...
	    else if ( ( alphaState(pivAlphaF(iP)) < 0 ) && ( stepAlpha.v(pivAlphaF(iP)) <= -zerotol() ) )
	    {
                distancetoedge = dalpha.v(pivAlphaF(iP)) - lb.v(pivAlphaF(iP));
	    }
	    ...
	    if ( ( distancetoedge >= zerotol() ) && ( stepAlpha.v(pivAlphaF(iP)) >= zerotol() ) )
	    {
                pscale = distancetoedge / stepAlpha.v(pivAlphaF(iP));
	    }

	    else if ( ( distancetoedge >= zerotol() ) && ( stepAlpha.v(pivAlphaF(iP)) <= -zerotol() ) )
	    {
                pscale = distancetoedge / -stepAlpha.v(pivAlphaF(iP));
	    }
...
template <class T, class S>
template <class U>
int optState<T,S>::fact_minverse(Vector<U> &aAlpha, Vector<U> &aBeta, const Vector<U> &bAlpha, const Vector<U> &bBeta, const Matrix<double> &Gp, const Matrix<double> &Gn, const Matrix<double> &Gpn) const
{
    (void) Gp;
...
template <class T, class S>
template <class U>
void optState<T,S>::fact_minvdiagsq(Vector<U> &ares, Vector<U> &bres, const Matrix<double> &Gn, const Matrix<double> &Gpn) const
{
    NiceAssert( Gn.isSquare() );
    NiceAssert( Gn.numCols() == Gpn.numCols() );
    NiceAssert( Gpn.numRows() == aN() );
    NiceAssert( Gpn.numCols() == bN() );
    NiceAssert( keepfact() );
    NiceAssert( ares.size() == aN() );
    NiceAssert( ares.size() == bN() );

    if ( ( probContext.fact_pfact(Gn,Gpn) == aNF() ) && ( probContext.fact_nfact(Gn,Gpn) == bNF() ) )
    {
        probContext.fact_minvdiagsq(ares,bres);
    }

    else
    {
        NiceThrow("Unable to disambiguate required inverse for singular case in diagonal inverse calculation.");
    }
}
...
void optState<T,S>::recalcAlphaGradhpzero(T &res, const Matrix<S> &GpGrad, const Matrix<double> &Gpn, const Vector<T> &gp, int i) const
{
    ...
    if ( ( alphaState(i) == -1 ) || ( alphaState(i) == +1 ) )
    {
        ...
        // Highly likely to have a kcache hit here, so use the faster version

        if ( aNLB() ) { res += sumb(temp,GpGrad(i,pivAlphaLB(),tmpva,tmpvb,tmpvd),dalpha(pivAlphaLB(),tmpvc)); }
        if ( aNF()  ) { res += sumb(temp,GpGrad(i,pivAlphaF (),tmpva,tmpvb,tmpvd),dalpha(pivAlphaF (),tmpvc)); }
        if ( aNUB() ) { res += sumb(temp,GpGrad(i,pivAlphaUB(),tmpva,tmpvb,tmpvd),dalpha(pivAlphaUB(),tmpvc)); }
    }
...
//phantomxyz
    ...
    res += sumb(temp,Gpn(i,pivBetaF(),tmpvd,tmpve,tmpvg),dbeta(pivBetaF(),tmpvf));
}
    ...
    if ( aNNZ() )
    {
        ...
        for ( i = 0 ; ( ( i < aN() ) && ( j < aNNZ() ) ) ; ++i )
        {
            if ( alphaState(i) )
            {
                ...
                twoProduct(gbi,Gpn(i,pivBetaF()),beta()(pivBetaF()));
