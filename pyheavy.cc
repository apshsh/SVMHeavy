...
/*
TO DO: get Dylan to put the weight in as prior on the objective model. Or even build
a BLK object that has the weight function built into it for speed. That way the weight
just works, which is ok. Or just a BLK that specifically defines a direct call to
python without faffing around with gentype etc.
*/
...
- add "if q(x)<0 then acquisition is zero" constraint to acquisition. q(x) is a python call from (call if feasibility check) Dylan that returns min_weight(x)-weight(x).
  Null (none) by default for no such function.
- add dynamic minimum on acquisition function to ensure min(a(x))>=0
...
- add symmetry constraint g(x) ~ y, g(Ax) ~ y (or G(x : Ax) = 0, which we could set up as a filter).
- add inequality constraint support for GPs (just set sigma so that the 95 percentile touches the posterior mean, or something like that?). Note that the current EP algorithm seems to fail when you have a combination of inequality and equality constraints in the same problem.
- add monotonicity constraints on axis (planar for simplicity)
