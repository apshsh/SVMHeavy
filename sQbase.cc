...
static const Vector<double> &crowcalc(int i, const void *Gphid, retVector<double> &tmp)
{
    ...
    if ( i >= aN )
    {
        return newGpsigma(i,tmp,tmpres);
    }
...
doitagain:
    ...
    {
        zeroeg.resize(x.betaGrad()(0).size()) = 0.0;
        ...
        Matrix<double> GpExtendTemplateOnDiag  = Gp(0,0);
...
doitagain:
    ...
    else
    {
        ...
        double GpExtendTemplateOffDiag = Gp(0,0);
        double GpExtendTemplateOnDiag  = Gp(0,0);
