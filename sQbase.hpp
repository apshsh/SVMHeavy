...
int testterm(double fnval, const Vector<double> &alpha)
{
    ...
    if ( ( maxitcnt > 0 ) && ( itcnt > (size_t) maxitcnt ) )
    {
...
public:
    stopCond()
    {
        ...
        runtimeend = DEFAULT_TRAINTIMEEND;
        maxfails   = DEFAULT_MAXFAILS;

        tol    = DEFAULT_OPTTOL;
        reltol = DEFAULTOUTERRELTOL;

        outermaxitcnt   = DEFAULTOUTERMAXITCNT;
        outermaxruntime = DEFAULTOUTERMAXTRAINTIME;
        outerruntimeend = DEFAULTOUTERTRAINTIMEEND;
        outermaxfails   = DEFAULT_MAXFAILS;

        outertol    = DEFAULTOUTERTOL;
        outerreltol = DEFAULTOUTERRELTOL;

        return;
    }

    stopCond(const stopCond &src)
    {
        maxitcnt   = src.maxitcnt;
        maxruntime = src.maxruntime;
        runtimeend = src.runtimeend;
        maxfails   = src.maxfails;

        tol    = src.tol;
        reltol = src.reltol;

        outermaxitcnt   = src.outermaxitcnt;
        outermaxruntime = src.outermaxruntime;
        outerruntimeend = src.outerruntimeend;
        outermaxfails   = src.outermaxfails;

        outertol    = src.outertol;
        outerreltol = src.outerreltol;

        return;
    }

    stopCond(const char *, stopCond &src)
    {
        // copy outer to inner

        maxitcnt   = src.outermaxitcnt;
        maxruntime = src.outermaxruntime;
        runtimeend = src.outerruntimeend;
        maxfails   = src.outermaxfails;

        tol    = src.outertol;
        reltol = src.outerreltol;
    ...
    stopCond &operator=(const stopCond &src)
    {
        maxitcnt   = src.maxitcnt;
        maxruntime = src.maxruntime;
        runtimeend = src.runtimeend;
    ...
    virtual ~stopCond() { return; }

    virtual void copyvars(stopCond *dest)
    {
        dest->maxitcnt   = maxitcnt;
        dest->maxruntime = maxruntime;
        dest->runtimeend = runtimeend;
        dest->maxfails   = maxfails;

        dest->tol    = tol;
        dest->reltol = reltol;

        dest->outermaxitcnt   = outermaxitcnt;
        dest->outermaxruntime = outermaxruntime;
        dest->outerruntimeend = outerruntimeend;
        dest->outermaxfails   = outermaxfails;

        dest->outertol    = outertol;
        dest->outerreltol = outerreltol;

        return;
    }

    // timeout: set if time has run out
    // bailout: error code for optimisation
    // stopout: set if other stopping criteria met

    bool stillrun(size_t itcnt, int isopt, int timeout, int bailout, int stopout)
    {
        return !isopt && ( ( itcnt < (size_t) maxitcnt ) || !maxitcnt ) && !timeout && !bailout && !stopout;
    }

    void testopt(size_t &itcnt, int &isopt, int &timeout, int &bailout, int &stopout, int &kickstart,
                 size_t feedback_cycle, size_t major_feedback_cycle,
                 time_used &start_time, time_used &curr_time,
                 const char *operdescr, double **uservars, const char **varnames, const char **vardescr, int goanyhow = 0)
    {
        (void) isopt;
        (void) bailout;
        (void) stopout;
        ...
        if ( maxruntime > 1 )
        {
            ...
            if ( TIMEDIFFSEC(curr_time,start_time) > maxruntime )
            {
                timeout = 1;
            }
        }

        else if ( runtimeend > 0 )
        {
            curr_time = TIMECALL;

            if ( TIMEABSSEC(curr_time) >= runtimeend )
            {
                timeout = 1;
            }
        }

        else if ( kickstart > maxfails )
        {
            errstream("\nOPTIMISATION FAILED!\n");
        ...
        {
            timeout = kbquitdet(operdescr,uservars,varnames,vardescr,goanyhow);
        }

        return;
    }

    double maxitcnt;    // max iterations
    double maxruntime;  // max train time
    double runtimeend;  // absolute training time end
    double maxfails;    // maximum number of consecutive "bad steps" before we give up

    double tol;    // optimality tolerance w.r.t. gradient
    double reltol; // reltol stopping condition 

    double outermaxitcnt;   // max iterations (outer loop)
    double outermaxruntime; // max train time (outer loop)
    double outerruntimeend; // absolute training time end (outer loop)
    double outermaxfails;   // maximum number of consecutive "bad steps" before we give up

    double outertol;    // optimality tolerance (outer loop)
    double outerreltol; // reltol stopping condition
};
...
public:
    ...
    fullOptState(optState<T,S> &_x,
        ...
        double (*_fixHigherOrderTerms)(fullOptState<T,S> &x, void *, const Vector<double> &, const Vector<double> &, double &) = nullptr, void *_htArg = nullptr,
        double _stepscalefactor = 1)
        : x(_x), Gpfull(_qGp), Gp(Gpfull(0,1,x.aN()-1,0,1,x.aN()-1,GpDerefer)), Gpsigmafull(_qGpsigma), Gpsigma(Gpsigmafull(0,1,x.aN()-1,0,1,x.aN()-1,GpsigmaDerefer)), Gn(_qGn), Gpn(_qGpn), 
    ...
    fullOptState(optState<T,S> &_x,
        ...
        double _stepscalefactor = 1)
        : x(_x), Gpfull(_qGp), Gp(Gpfull(0,1,x.aN()-1,0,1,x.aN()-1,GpDerefer)), Gpsigmafull(_qGpsigma), Gpsigma(Gpsigmafull(0,1,x.aN()-1,0,1,x.aN()-1,GpsigmaDerefer)), Gn(_qGn), Gpn(_qGpn), 
          gp(_gp), gn(_gn), hp(_hp), lb(_lb), ub(_ub), GpnRowTwoMag(_qGpnRowTwoMag)
    {
        GpnRowTwoMagdef.resize(gp.size());
        hpdef.resize(gp.size());
...
//private:
    ...
    {
        NiceThrow("Solver not defined at base");
    ...
    double intfullfixbasea(fullOptState<double,double> &xx, void *p, const Vector<double> &, const Vector<double> &, double &) const
    {
        ...
        res += ((double) innerProduct(temp,alpha(aLB,tmpva),Gp(aLB,aLB,tmpma)*alpha(aLB,tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aLB,tmpva),Gp(aLB,aUB,tmpma)*alpha(aUB,tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aLB,tmpva),Gp(aLB,aF, tmpma)*alpha(aF, tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aUB,tmpva),Gp(aUB,aLB,tmpma)*alpha(aLB,tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aUB,tmpva),Gp(aUB,aUB,tmpma)*alpha(aUB,tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aUB,tmpva),Gp(aUB,aF, tmpma)*alpha(aF, tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aF, tmpva),Gp(aF ,aLB,tmpma)*alpha(aLB,tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aF, tmpva),Gp(aF ,aUB,tmpma)*alpha(aUB,tmpvb)))/2.0;
        res += ((double) innerProduct(temp,alpha(aF, tmpva),Gp(aF ,aF, tmpma)*alpha(aF, tmpvb)))/2.0;

        res += ((double) innerProduct(temp,alpha(aLB,tmpva),gp(aLB,tmpvb)));
        res += ((double) innerProduct(temp,alpha(aUB,tmpva),gp(aUB,tmpvb)));
        res += ((double) innerProduct(temp,alpha(aF ,tmpva),gp(aF ,tmpvb)));
