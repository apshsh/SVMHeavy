...
//    }
    ...
    if ( ( chinum < aN ) && useGpBase )
    {
        ...
        if ( x.keepfact() )
        {
            ...
            fullOptStateActive xxxx(x,Gp,Gpsigma,Gn,Gpn,gp,gn,hp,lb,ub,nullptr,nullptr,1.0);
        ...
        else
        {
            ...
            fullOptStateD2C xxxx(x,Gp,Gpsigma,Gn,Gpn,gp,gn,hp,lb,ub,nullptr,nullptr,1.0);
...
//errstream() << "sum alpha start = " << sum(alpha) << "\n";
    ...
    xxx.initGradBeta(locGp(0,1,aN-1,0,1,aN-1,tmpma),locGp(0,1,aN-1,0,1,aN-1,tmpmb),locGn,locGpn,locgp,locgn,lochp);
    xxx.refreshGrad(locGp(0,1,aN-1,0,1,aN-1,tmpma),locGn,locGpn,locgp,locgn,lochp);
//errstream() << "fn start = " << fnnext << "\n";
    ...
    stopCond outsc("out",sc);

    double outlr = lr;
    double outlrback = lrback;
    double outdelta = delta;
    double *uservars[] = { &(outsc.maxitcnt), &(outsc.maxruntime), &(outsc.runtimeend), &outlr, &outlrback, &outdelta, nullptr };
    const char *varnames[] = { "outermaxitcnt", "outermaxtraintime", "outtraintimeend", "outerlr", "outerlrback", "outerdelta", nullptr };
    const char *vardescr[] = { "Outer maximum iteration count (0 for unlimited)", "Outer maximum training time (seconds, 0 for unlimited)", "Outer abs train time end (rel, sec, -1 for na)", "Outer learning rate", "Outer learning rate backpull", "delta", nullptr };
    ...
    errstreamunlogged() << " #\b\b";

    while ( !killSwitch && outsc.stillrun(itcnt,isopt,timeout,bailout,stopout) )
    {
        ...
        if ( !isopt )
        {
            if ( usels & 0x01 )
            {
                ...
                calcStep(aN,bN,hpzero,sc.outertol,useactive,
                         sc,//maxitcnt,maxruntime,runtimeend,
...
//FIXME: add terminate on modstep too small, repeated lack of progress in fnnext
        ...
        outsc.testopt(itcnt,isopt,timeout,bailout,stopout,kickstart,FEEDBACK_CYCLE,MAJOR_FEEDBACK_CYCLE,start_time,curr_time,"quadratic (sQsLsAsWs) optimisation",uservars,varnames,vardescr);
    }
...
//errstream() << "locgn = " << locgn << "\n";
    ...
    while ( badres )
    {
        for ( i = 0 ; i < aN ; ++i )
        {
            if ( xxx.alphaState()(i) == -2 )
            {
                ...
                xxx.modAlphaLBtoLF(xxx.findInAlphaLB(i),locGp(0,1,aN-1,0,1,aN-1,tmpma),locGp(0,1,aN-1,0,1,aN-1,tmpmb),locGn,locGpn,locgp,locgn,lochp);
            }
            ...
            else if ( xxx.alphaState()(i) == +2 )
            {
                ...
                xxx.modAlphaUBtoUF(xxx.findInAlphaUB(i),locGp(0,1,aN-1,0,1,aN-1,tmpma),locGp(0,1,aN-1,0,1,aN-1,tmpmb),locGn,locGpn,locgp,locgn,lochp);
            }
            ...
            if ( xxx.alphaState()(i) )
            {
                ...
                xxx.alphaStep(i,astep,locGp(0,1,aN-1,0,1,aN-1,tmpma),locGn,locGpn,locgp,locgn,lochp);
            }
        }
        ...
        for ( i = 0 ; i < bN ; ++i )
        {
            if ( xxx.betaState()(i) )
            {
                ...
                xxx.betaStep(i,bstep,locGp(0,1,aN-1,0,1,aN-1,tmpma),locGn,locGpn,locgp,locgn,lochp);
            }
        }
        ...
        {           
            ...
            xxx.refact(locGp(0,1,aN-1,0,1,aN-1,tmpma),locGp(0,1,aN-1,0,1,aN-1,tmpmb),locGn,locGpn,locgp,locgn,lochp);

            if ( useactive )
            {
                // Memory intensive, keeps factorisation (slow update), but can do arbitrary constraints

                fullOptStateActive xxxx(xxx,locGp,locGpsigma,locGn,locGpn,locgp,locgn,lochp,loclb,locub,nullptr,nullptr,1.0);
            ...
            else
            {
                ...
                fullOptStateD2C xxxx(xxx,locGp,locGpsigma,locGn,locGpn,locgp,locgn,lochp,loclb,locub,nullptr,nullptr,1.0);
    ...
    if ( scale < 1.0 )
    {
        ...
        xxx.scale(scale,locGp(0,1,aN-1,0,1,aN-1,tmpma),locGn,locGpn,locgp,locgn,lochp);
    }
