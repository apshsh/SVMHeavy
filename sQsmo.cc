...
//const Vector<double> &gn, const Vector<double> &hp, const Vector<double> &lb, const Vector<double> &ub, int maxitcnt, double maxtraintime, double traintimeend)
...
//    double stepscale = stepscalefactor;
	...
	//res = solve_SMO(killSwitch,x,Gp,Gpsigma,Gn,Gpn,gp,gn,hp,lb,ub,maxitcnt,maxruntime,runtimeend,just_zero_f,nullptr,nullptr,1.0);
	res = solve_SMO(killSwitch,x,Gp,Gpsigma,Gn,Gpn,gp,gn,hp,lb,ub,sc,just_zero_f,nullptr,nullptr,1.0);
    }

    else
    {
	//res = solve_SMO_fixed_bias(killSwitch,x,Gp,Gpsigma,Gn,Gpn,gp,gn,hp,lb,ub,maxitcnt,maxruntime,runtimeend,nullptr,nullptr,1.0);
	res = solve_SMO_fixed_bias(killSwitch,x,Gp,Gpsigma,Gn,Gpn,gp,gn,hp,lb,ub,sc,nullptr,nullptr,1.0);
    }
...
int solve_SMO(svmvolatile int &killSwitch, optState<double,double> &probdef,
                       ...
                       stopCond sc, //size_t maxitcntint, double maxtraintime, double traintimeend,
                       int just_zero_f, double (*fixHigherOrderTerms)(optState<double,double> &x, void *, const Vector<double> &, const Vector<double> &, double &), void *htArg, double stepscalefactor)
{
    int f_zero = 0;
    int i;
    int numChanged = 0;
...
//    }
        ...
        for ( i = 0 ; i < probdef.aN() ; ++i )
	{
        ...
        sc.testopt(itcnt,isopt,timeout,bailout,stopout,kickstart,FEEDBACK_CYCLE,MAJOR_FEEDBACK_CYCLE,start_time,curr_time,"SMO (sQsmo) optimisation",uservars,varnames,vardescr);
    }
