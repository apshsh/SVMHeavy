...
public:
    ...
    fullOptStateSMOVect(optState<Vector<double>,S> &_x,
        ...
        int _zcmaxitcnt = DEFZCMAXITCNT, int _inmaxitcnt = DEFINMAXITCNT,
        double _kappa = DEFAULT_OPTTOL, double _stol = DEFAULT_ZTOL, double _iota = DEFAULT_ZTOL, int _vdim = -1)
        : fullOptState<Vector<double>,S>(_x,_qGp,_qGpsigma,_qGn,_qGpn,_gp,_gn,_hp,_lb,_ub,_fixHigherOrderTerms,_htArg,_stepscalefactor)
    {
    ...
    fullOptStateSMOVect(optState<Vector<double>,S> &_x,
        ...
        int _zcmaxitcnt = DEFZCMAXITCNT, int _inmaxitcnt = DEFINMAXITCNT,
        double _kappa = DEFAULT_OPTTOL, double _stol = DEFAULT_ZTOL, double _iota = DEFAULT_ZTOL, int _vdim = -1)
        : fullOptState<Vector<double>,S>(_x,_qGp,_qGpsigma,_qGn,_qGpn,_gp,_gn,_lb,_ub,_fixHigherOrderTerms,_htArg,_stepscalefactor)
    {
        NiceAssert( !_fixHigherOrderTerms );
    ...
    fullOptStateSMOVect(optState<Vector<double>,S> &_x,
        ...
        int _zcmaxitcnt = DEFZCMAXITCNT, int _inmaxitcnt = DEFINMAXITCNT,
        double _kappa = DEFAULT_OPTTOL, double _stol = DEFAULT_ZTOL, double _iota = DEFAULT_ZTOL, int _vdim = -1)
        : fullOptState<Vector<double>,S>(_x,_qGp,_qGpsigma,_qGn,_qGpn,_gp,_gn,_hp,_lb,_ub,_qGpnRowTwoMag,_fixHigherOrderTerms,_htArg,_stepscalefactor)
    {
        NiceAssert( !_fixHigherOrderTerms );
        ...
        const Vector<double> &_lb, const Vector<double> &_ub, const Vector<double> &_qGpnRowTwoMag)
    {
        ...
        MEMNEW(res,fullOptStateSMOVect<S>(fullOptState<Vector<double>,S>::x,_qGp,_qGpsigma,_qGn,_qGpn,_gp,_gn,_hp,_lb,_ub,_qGpnRowTwoMag));
...
int fullOptStateSMOVect<S>::solve(svmvolatile int &killSwitch)
{
    if ( vdim == -1 )
    {
        vdim = (fullOptState<Vector<double>,S>::gn)(0).size();
    }

    NiceAssert( !(fullOptState<Vector<double>,S>::GpnRowTwoSigned) );
    NiceAssert( vdim == (fullOptState<Vector<double>,S>::gn)(0).size() );
...
//    NiceAssert( !(fullOptState<Vector<double>,S>::fixHigherOrderTerms) );
    ...
    return solve_quadratic_program_smoVect(killSwitch,fullOptState<Vector<double>,S>::x,
        ...
        fullOptState<Vector<double>,S>::sc, //fullOptState<Vector<double>,S>::maxitcnt, fullOptState<Vector<double>,S>::maxruntime, fullOptState<Vector<double>,S>::runtimeend,
...
//errstream() << "\n\n\nphantomx: call outer loop t = " << pad.t << "\n";
        //isopt = optSMOVect(killSwitch,op,Gp,GpGrad,Gpsigma,Gn,Gpn,gp,gn,hpeff,ubeff,maxitcnt,maxtraintime,traintimeend,pad,vdim);
        isopt = optSMOVect(killSwitch,op,Gp,GpGrad,Gpsigma,Gn,Gpn,gp,gn,hpeff,ubeff,sc,pad,vdim);
    }
               ...
               smoVectScratch<T> &pad, int vdim)
{
    ...
    double *uservars[] = { &(sc.maxitcnt), &(sc.maxruntime), &(sc.runtimeend), nullptr };
    const char *varnames[] = { "maxitcnt", "maxruntime", "runtimeend", nullptr };
    const char *vardescr[] = { "Maximum iteration count (0 for unlimited)", "Maximum training time (seconds, 0 for unlimited)", "Absolute training time end (relative, secs, -1 for na)", nullptr };
...
//errstream() << "phantomxa: eup = " << eup(0,1,Nposs-1) << "\n";
//errstream() << "phantomxa: epivot = " << epivot(0,1,Nposs-1) << "\n";
//errstream() << "phantomxa: g = " << g(epivot(0,1,Nposs-1)) << "\n";
//errstream() << "phantomxa: alpha = " << (op.alpha())(epivot(0,1,Nposs-1)) << "\n";
//errstream() << "phantomxa: alphaGrad = " << (op.alphaGrad())(epivot(0,1,Nposs-1)) << "\n";
...
//errstream() << "phantomxa: Sorting gradients\n";
            ...
            for ( i = 0 ; ( i < Nposs-1 ) && ( stepfound < STEPSPERBLOCK ) ; ++i )
            {
                ...
                for ( j = 0 ; j < Nposs-1-i ; ++j )
                {
                    ...
                    fm("&",l) = em(k)+em(l)-(2*twoProduct(temp,g(k),g(l)));
                    fup("&",j) = fm(l);
                }
...
//errstream() << "phantomx 11 " << stepfound << "\n";
        ...
        sc.testopt(itcnt,isopt,timeout,bailout,stopout,kickstart,FEEDBACK_CYCLE,MAJOR_FEEDBACK_CYCLE,start_time,curr_time,"Vectorial SMO (sQsmoVect) optimisation",uservars,varnames,vardescr);
    }
...
int stepFind(smoVectScratch<T> &pad, int testdiscontinuities)
{
    ...
    pad.alphaijprod       = twoProduct(temp,pad.alphai,pad.alphaj);
...
//errstream() << "phantomxc 04 " << pad.zetaj << "\n";
        ...
        pad.ki = twoProduct(temp,pad.alphai,pad.g0);
        pad.kj = twoProduct(temp,pad.alphaj,pad.g0);
...
//errstream() << "phantomxc 1\n";
        ...
        pad.DeltaAlpha = pad.g0;
        (pad.DeltaAlpha).negate();
        (pad.DeltaAlpha).scaleAdd(twoProduct(temp,pad.alphai,pad.g0)/(((pad.etatilde)/(pad.betai))+(pad.normalphai)),pad.alphai);
        pad.DeltaAlpha /= pad.etatilde;
    }
...
//errstream() << "phantomxc 2\n";
        ...
        pad.DeltaAlpha = pad.g0;
        (pad.DeltaAlpha).negate();
        (pad.DeltaAlpha).scaleAdd(twoProduct(temp,pad.alphaj,pad.g0)/(((pad.etatilde)/(pad.betaj))+(pad.normalphaj)),pad.alphaj);
        pad.DeltaAlpha /= pad.etatilde;
    }
...
//errstream() << "\nphantomxqq 0\n";
    ...
    pad.thetai   = twoProduct(temp,pad.alphai,pad.DeltaAlpha)/(pad.xabs);
    pad.thetaj   = twoProduct(temp,pad.alphaj,pad.DeltaAlpha)/(pad.xabs);
    pad.tildeeij = twoProduct(temp,pad.eij,   pad.DeltaAlpha)/(pad.xabs);
