...
int SSV_Scalar::entrain(SVM_Scalar &zzmodel, SVM_Scalar &betamodel, int &res, svmvolatile int &killSwitch)
{
    ...
    for ( i = 0 ; i < Nzs() ; ++i )
    {
        ...
        if ( ( z(i).indsize() != z(i).size() ) || ( z(i).indsize() != zdim ) )
        {
            for ( j = 0 ; j < zdim ; ++j )
            {
                randufill(rnval); // uniform random (0,1)
    ...
    betaR = (betamodel.alphaR())(0,1,Nzs()-1,tmpva);
...
//errstream() << "phantomx M++ = " << M()(0,1,Nzs()-1,0,1,Nzs()-1) << "\n";
//errstream() << "phantomx M-+ = " << M()(Nzs(),0,1,Nzs()-1) << "\n";
//errstream() << "phantomx M+- = " << M()(0,1,Nzs()-1,Nzs(),"&") << "\n";
//errstream() << "phantomx M-- = " << M()(Nzs(),Nzs()) << "\n";
//errstream() << "phantomx n+ = " << n()(0,1,Nzs()-1) << "\n";
...
//    int dummyind = -1;
    ...
    const char *vardescr[] = { "Maximum iteration count (0 for unlimited)", "Maximum training time (seconds, 0 for unlimited)", "Learning rate", "Momentum factor", "Zero tolerance", "Over-step scaleback factor", nullptr };
...
//errstream() << "\n\n";
        ...
        for ( i = 0 ; i < Nzs() ; ++i )
        {
            if ( i != j )
            {
                zstep("&",j).scaleAdd(-lr*zgrad(i+N())/(  static_cast<double>(((itcnt+1)*(itcnt+1))*(Nzs()-1))  ),zprev(j));
                zstep("&",j).scaleAdd(-lr*xgrad(i+N())/(  static_cast<double>(((itcnt+1)*(itcnt+1))*(Nzs()-1))  ),zprev(i));
            }
        }
