...
int SVM_Binary::addTrainingVector(int i, const gentype &z, const SparseVector<gentype> &x, double Cweigh, double epsweigh, int dval)
{
    (void) dval;
...
int SVM_Binary::qaddTrainingVector(int i, const gentype &z, SparseVector<gentype> &x, double Cweigh, double epsweigh, int dval)
{
    (void) dval;
...
int SVM_Binary::addTrainingVector(int i, const Vector<int> &xd, const Vector<SparseVector<gentype> > &xx, const Vector<double> &xCweigh, const Vector<double> &xepsweigh, const Vector<double> &xz)
{
    ...
    //if ( xd.size() )
    {
        Vector<double> zvals(xd.size());
        Vector<double> cwvals(xd.size());
        Vector<int> dvals(xd.size());
        ...
        for ( int j = 0 ; j < xd.size() ; ++j )
        {
            ...
            zvals("&",j)  = ZCALC(bintraintarg(i+j),bintrainclass(i+j),bineps,binepsclass,bintrainclass(i+j),binepsweight(i+j));
            cwvals("&",j) = CWCALC(bintrainclass(i+j),binCweight(i+j),binCweightfuzz(i+j),dthres);
            dvals("&",j)  = DCALC(bintrainclass(i+j));
        }

        res |= SVM_Scalar::addTrainingVector(i,zvals,xx,cwvals,xepsweigh,dvals);

        for ( int j = 0 ; j < xd.size() ; ++j )
        {
//            res |= SVM_Scalar::addTrainingVector(i+j,zvals(j),xx(j),cwvals(j),xepsweigh(j),dvals(j));
...
int SVM_Binary::qaddTrainingVector(int i, const Vector<int> &xd, Vector<SparseVector<gentype> > &xx, const Vector<double> &xCweigh, const Vector<double> &xepsweigh, const Vector<double> &xz)
{
    ...
    //if ( xd.size() )
    {
        ...
        res |= SVM_Scalar::qaddTrainingVector(i,zvals,xx,cwvals,xepsweigh,dvals);

        for ( int j = 0 ; j < xd.size() ; ++j )
        {
//            res |= SVM_Scalar::qaddTrainingVector(i+j,zvals(j),xx("&",j),cwvals("&",j),xepsweigh(j),dvals(j));
...
double &SVM_Binary::dedgTrainingVector(double &res, int i) const
{
...
double &SVM_Binary::d2edg2TrainingVector(double &res, int i) const
{
//    double res = 0;
...
Matrix<double> &SVM_Binary::dedKTrainingVector(Matrix<double> &res) const
{
    res.resize(N(),N());
...
Vector<double> &SVM_Binary::dedKTrainingVector(Vector<double> &res, int i) const
{
    NiceAssert( i < N() );
