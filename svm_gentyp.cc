...
SVM_Gentyp::SVM_Gentyp() : SVM_Vector()
{
    ...
    L.fudgeOn(); // This ensures order is simple
...
int SVM_Gentyp::setBasisUU(const Vector<gentype> &o)
{
    ...
    {
        L.remove(i,M(0,1,i-1,0,1,i-1,tmpma),dummyGn,dummyGpn(0,1,i-1,0,1,-1,tmpmb));
    }
    ...
    {
        L.add(i,1,M(0,1,i,0,1,i,tmpma),dummyGn,dummyGpn(0,1,i,0,1,-1,tmpmb));
    }
...
void SVM_Gentyp::calcMxy(double &res, const gentype &x, const gentype &y) const
{
    ...
    res = getUUOutputKernel().K2(xx,yy,xxinfo,yyinfo,0_gent);
