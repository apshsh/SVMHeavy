...
#define QUADCOSTDIAGOFFSET(_xclass_,_qCweigh_,_qCweighfuzz_)  ( (_xclass_) ? ( isQuadraticCost() ? (1/(CNval*(_qCweigh_)*(_qCweighfuzz_)*mulCclass(label_placeholder.findID(_xclass_)))) : 0.0 ) : 0.0 )
...
double SVM_MultiC_atonce::CWEIGH_CALC(int _xclass_, double _qCweigh_, double _qCweighfuzz_, int _s_, double _dthres_)
{
    // _xclass_ = y_i (training class of the vector in question)
    // _s_  = element of epsilon subvector under consideration.

    double res = _qCweigh_*_qCweighfuzz_*mulCclass(label_placeholder.findID(_xclass_));
...
/*
double SVM_MultiC_atonce::CWEIGH_CALCBASE(int _xclass_, double _qCweigh_, double _qCweighfuzz_, int _s_, double _dtrehs_)
{
    // _xclass_ = y_i (training class of the vector in question)
    // _s_  = element of epsilon subvector under consideration.
...
res = realOwner->K2(idiv,jdiv,pxyprod);
...
int SVM_MultiC_atonce::settraintimeend(double traintimeend)
{
    QA.settraintimeend(traintimeend);
    Q.settraintimeend(traintimeend);
...
int SVM_MultiC_atonce::setLinBiasForceclass(int d, double newval)
{
    ...
    if ( isrecdiv() )
    {
        ...
        linbiasforcered -= linbiasforceval(0);
...
int SVM_MultiC_atonce::setmaxwins(void)
{
	...
	if ( numClasses() >= 1 )
	{
		...
		for ( i = 0 ; i < N() ; ++i )
		{
            ...
            Q.setbiasdim(numClasses()+1,0,db(0));
...
int SVM_MultiC_atonce::addTrainingVector(int i, const gentype &z, const SparseVector<gentype> &x, double Cweigh, double epsweigh, int dval)
{
    (void) dval;
...
int SVM_MultiC_atonce::qaddTrainingVector(int i, const gentype &z, SparseVector<gentype> &x, double Cweigh, double epsweigh, int dval)
{
    (void) dval;
...
//    kerndiagval.add(i); kerndiagval("&",i) = kerndiagis;
	...
	if ( q != MODPOS(y) )
	{
            ...
            Gpn("&",IQFIX(y,i,q),tmpva,tmpvd) = CMcl(label_placeholder.findID(y)+1,( isrecdiv() ? 1 : 0 ))(QFIX(y,q),tmpvb,tmpvc);

            if ( xisrankorgrad(i) )
            {
                Gpn("&",IQFIX(y,i,q),tmpva,tmpvd) *= 0.0;
            }
...
int SVM_MultiC_atonce::train(int &res, svmvolatile int &killSwitch)
{
    ...
    if ( numClasses() )
    {
        ...
        int tempres = 0;
        result = loctrain(tempres,killSwitch,realN,0);
        res *= 100;
        res += tempres;
...
Vector<double> &SVM_MultiC_atonce::dedgTrainingVector(Vector<double> &res, int i) const
{
...
Matrix<double> &SVM_MultiC_atonce::dedKTrainingVector(Matrix<double> &res) const
{
    res.resize(N(),N());
...
Vector<double> &SVM_MultiC_atonce::dedKTrainingVector(Vector<double> &res, int i) const
{
    NiceAssert( i < N() );
...
int SVM_MultiC_atonce::gTrainingVector(Vector<double> &gprojectExt, int &locclassrep, int i, int retaltg, gentype ***pxyprodi) const
{
...
int SVM_MultiC_atonce::gTrainingVector(Vector<gentype> &gprojectExt, int &locclassrep, int i, int retaltg, gentype ***pxyprodi) const
{
    NiceAssert( !( retaltg & 2 ) );
    ...
    int res = gTrainingVector(gproject,locclassrep,i,retaltg,pxyprodi);
...
//            kerndiagval += bp(0,numClasses()-1,(N()*(numClasses()-1))-1);
...
//    input >> dummy; input >> kerndiagval;
    ...
    MEMNEW(Gpval  ,Matrix<double>(Kcache_celm_v_double,Kcache_celm_double,Kcache_crow_double,(void *) &(kerncache), (N())*((numClasses()-1)),(N())*((numClasses()-1))));
    MEMNEW(Gpsigma,Matrix<double>(Kcache_celm_v_double,Kcache_celm_double,Kcache_crow_double,(void *) &(sigmacache),(N())*((numClasses()-1)),(N())*((numClasses()-1))));
...
Vector<double> &updim(const Vector<Vector<double> > &u, const Vector<double> &dTDimArg, Vector<double> &nDimArg)
{
    ...
    for ( s = 0 ; s < u.size() ; ++s )
    {
        nDimArg("&",s) = twoProduct(temp,u(s),dTDimArg);
    }
