...
int SVM_MultiC_redbin::settraintimeend(double traintimeend)
{
    int q;
    int res = 0;
    ...
    for ( q = 0 ; q < Q.size() ; ++q )
    {
        res |= Q("&",q).settraintimeend(traintimeend);
    }

    res |= QA.settraintimeend(traintimeend);
...
int SVM_MultiC_redbin::addclass(int label, int epszero)
{
	...
	if ( label_placeholder.findID(label) == -1 )
	{
		...
		classRepval("&",nnew-1)("&",0,1,mold-1,tmpva)    = 0;
...
int SVM_MultiC_redbin::train(int &res, svmvolatile int &killSwitch)
{
    ...
    if ( tspaceDim() )
    {
        double dthres = Q(0).rejectThreshold();
...
Vector<double> &SVM_MultiC_redbin::dedgTrainingVector(Vector<double> &res, int i) const
{
...
Matrix<double> &SVM_MultiC_redbin::dedKTrainingVector(Matrix<double> &res) const
{
    res.resize(N(),N());
...
Vector<double> &SVM_MultiC_redbin::dedKTrainingVector(Vector<double> &res, int i) const
{
    NiceAssert( i < N() );
...
int SVM_MultiC_redbin::gTrainingVector(Vector<double> &gproject, int &locclassrep, int i, int retaltg, gentype ***pxyprodi) const
{
