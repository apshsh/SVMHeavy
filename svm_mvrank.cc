...
//TRAINING: why not start with all u the same, then let them drift apart only as far as required to achieved optimality?  Maybe start with all equal or something? REASON: if all u same then step is zero 
            ...
            for ( r = 0 ; r < M ; ++r )
            {
                for ( i = 0 ; i < N() ; ++i )
                {
                    for ( j = 0 ; j < N() ; ++j )
                    {
                        ...
                        {
                            NiceAssert( x(i).isf4indpresent(7) );
                            NiceAssert( x(j).isf4indpresent(7) );

                            if ( ( (int) x(i).f4(7) == q ) && ( (int) x(j).f4(7) == r ) )
                            {
...
int SVM_MvRank::train(int &res, svmvolatile int &killSwitch)
{
    ...
    double stepscalefactor = lrmvrank();
    double outertol = ztmvrank();
    double *uservars[] = { &maxitcntval, &xmtrtime, &stepscalefactor, &beta, &outertol, nullptr };
    const char *varnames[] = { "itercount", "traintime", "stepscale", "beta", "outertol", nullptr };
    const char *vardescr[] = { "Maximum iteration count (0 for unlimited)", "Maximum training time (seconds, 0 for unlimited)", "Step scale", "beta factor", "Outer optimality tolerance", nullptr };
