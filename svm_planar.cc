...
SVM_Planar::SVM_Planar() : SVM_Scalar()
{
    ...
    SVM_Scalar::setGpnExt(nullptr,&inGpn);
...
double SVM_Planar::K2(int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo, int resmode) const
{
    double res = 0;
    ...
    if ( midadd || ( i < 0 ) || ( j < 0 ) || resmode )
    {
        ...
        res = cheatscache.K2(i,j,pxyprod,xx,yy,xxinfo,yyinfo,resmode);
    }
    ...
    const SparseVector<gentype> *xinear      = nullptr;
    const SparseVector<gentype> *xifar       = nullptr;
    const SparseVector<gentype> *xifarfar    = nullptr;
    const SparseVector<gentype> *xifarfarfar = nullptr;
    const vecInfo *xinearinfo = nullptr;
    const vecInfo *xifarinfo  = nullptr;
    const gentype *ineartup = nullptr;
    const gentype *ifartup  = nullptr;

    int inear,ifar,iokr,iok,idiagr,igradOrder,iplanr,iplan,iset,idenseint,idensederiv,alr,arr,agr;

    const SparseVector<gentype> *xjnear      = nullptr;
    const SparseVector<gentype> *xjfar       = nullptr;
    const SparseVector<gentype> *xjfarfar    = nullptr;
    const SparseVector<gentype> *xjfarfarfar = nullptr;
    const vecInfo *xjnearinfo = nullptr;
    const vecInfo *xjfarinfo  = nullptr;
    const gentype *jneartup = nullptr;
    const gentype *jfartup  = nullptr;

    int jnear,jfar,jokr,jok,jdiagr,jgradOrder,jplanr,jplan,jset,jdenseint,jdensederiv,blr,brr,bgr;
    ...
    int loctanga = detangle_x(xauntang,xainfountang,xinear,xifar,xifarfar,xifarfarfar,xinearinfo,xifarinfo,inear,ifar,ineartup,ifartup,alr,arr,agr,xagrR,iokr,iok,arankL,arankR,agmuL,agmuR,i,idiagr,&x(i),xxinfo,igradOrder,agradOrderR,iplanr,iplan,iset,idenseint,idensederiv,sumind,sumweight,iadiagoffset,iavectset,1,0);
    int loctangb = detangle_x(xbuntang,xbinfountang,xjnear,xjfar,xjfarfar,xjfarfarfar,xjnearinfo,xjfarinfo,jnear,jfar,jneartup,jfartup,blr,brr,bgr,xbgrR,jokr,jok,brankL,brankR,bgmuL,bgmuR,j,jdiagr,&x(j),yyinfo,jgradOrder,bgradOrderR,jplanr,jplan,jset,jdenseint,jdensederiv,sumind,sumweight,ibdiagoffset,ibvectset,1,0);
...
int SVM_Planar::rankcalcGpn(Vector<double> &res, int d, const SparseVector<gentype> &x, int i)
{
    ...
    if ( x.isf1offindpresent() || x.isf4indpresent(1) || !(x.isf4indpresent(7)) )
    {
        nzres = 0;
        res = 0.0;
    }

    else if ( (x.f4(7)).isValInteger() )
    {
        retVector<double> tmpva;
        retVector<double> tmpvb;

        nzres = 1;
        res = VV((int) x.f4(7),tmpva,tmpvb);
    }
    ...
    else
    {
        ...
        for ( j = 0 ; j < bdim ; ++j )
        {
            res("&",j) = (double) (VbasisVV()(j)*(x.f4(7)));
        }
    }
...
{
    twoProduct(res,locbasis(i),locbasis(j));
...
void SVM_Planar::refactorVV(int updateGpn)
{
    ...
    if ( updateGpn && !isFixedBias() && SVM_Planar::N() && NbasisVV() )
    {
        ...
        for ( i = 0 ; i < SVM_Planar::N() ; ++i )
        {
            rankcalcGpn(inGpn("&",i,tmpva,tmpvb),locd(i),x(i),i);
        }
...

    {
        ...
        if ( NS() )
        {
            for ( j = 0 ; j < SVM_Planar::N() ; ++j )
            {
                if ( alphaState()(j) )
                {
                    K2(temp,i,j,pxyprodi ? (const gentype **) pxyprodi[j] : nullptr);
                    temp *= alphaR()(j);
        ...
        if ( !isFixedBias() && NbasisVV() )
        {
            ...
            {
                if ( x(i).isf4indpresent(7) && x(i).f4(7).isValInteger() )
                {
                    temp =  VbasisVV()(j)*VbasisVV()((int) x(i).f4(7));
                    temp *= SVM_Scalar::biasVMulti(j);

                    resg += temp;
                }

                else if ( x(i).isf4indpresent(7) )
                {
                    temp =  VbasisVV()(j)*(x(i).f4(7));
                    temp *= SVM_Scalar::biasVMulti(j);
...
double SVM_Planar::calcDist(const gentype &ha, const gentype &hb, int ia, int db) const
{
    ...
    else if ( ia >= 0 )
    {
        ...
        haa *= x(ia).f4(7).isValInteger() ? VbasisVV()((int) x(ia).f4(7)) : x(ia).f4(7);
...
{ 
    ...
    inGpn.addRow(i);
    rankcalcGpn(inGpn("&",i,tmpva,tmpvb),d,xx,i);
