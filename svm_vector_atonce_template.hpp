...
public:
    ...
    virtual int NS (void)  const override { return Q.aNUF();        }
    virtual int NZ (void)  const override { return Q.aNZ();         }
    virtual int NF (void)  const override { return NS();            }
    virtual int NC (void)  const override { return NZ();            }
    virtual int NLB(void)  const override { return 0;               }
    virtual int NLF(void)  const override { return 0;               }
    virtual int NUF(void)  const override { return NF();            }
    virtual int NUB(void)  const override { return 0;               }
    ...
    virtual int tspaceDim(void)  const override { return biasV().size();  }
    ...
    virtual int isOptActive(void) const override { return 1; }
    virtual int isOptSMO(void)    const override { return 0; }
    virtual int isOptD2C(void)    const override { return 0; }
    virtual int isOptGrad(void)   const override { return 0; }
    ...
    virtual double Opttolb     (void) const override { return opttolval;               }
    virtual double Opttolc     (void) const override { return opttolval;               }
    virtual double Opttold     (void) const override { return opttolval;               }
    virtual double lr          (void) const override { return DEFAULT_LR;              }
    virtual double lrb         (void) const override { return DEFAULT_LR;              }
    virtual double lrc         (void) const override { return DEFAULT_LR;              }
    virtual double lrd         (void) const override { return DEFAULT_LR;              }
    ...
    virtual int isKreal(void)   const override;
    virtual int isKunreal(void) const override;
    ...
    virtual int setOptActive(void) override { return 0; }
    virtual int setOptSMO(void)    override { return 0; }
    virtual int setOptD2C(void)    override { return 0; }
    virtual int setOptGrad(void)   override { return 0; }
    ...
    virtual void fudgeOn(void)  override { Q.fact_fudgeOn(*GpGrad,Gn,Gpn);  return; }
    virtual void fudgeOff(void) override { Q.fact_fudgeOff(*GpGrad,Gn,Gpn); return; }
...
protected:
    ...
    const Matrix<T> &getGpGradTemp(void) const { return *GpGrad; }
...
template <class T> double norm2(const SVM_Vector_atonce_temp<T> &a);
template <class T> double abs2 (const SVM_Vector_atonce_temp<T> &a);

template <class T> double norm2(const SVM_Vector_atonce_temp<T> &a) { return a.RKHSnorm(); }
template <class T> double abs2 (const SVM_Vector_atonce_temp<T> &a) { return a.RKHSabs();  }
...
template <class T>
void SVM_Vector_atonce_temp<T>::fillCache(int Ns, int Ne)
{
    Ne = ( Ne >= 0 ) ? Ne : (*GpGrad).numRows()-1;
...
template <class T>
int SVM_Vector_atonce_temp<T>::settraintimeend(double xtraintimeend)
{
    traintimeendval = xtraintimeend;
...
int SVM_Vector_atonce_temp<T>::settspaceDim(int newdim)
{
    ...
    if ( newdim != tspaceDim() )
    {
        ...
        if ( newdim > olddim )
        {
            ...
            gn("&",0)("&",olddim,1,newdim-1,tmpva) = 0.0;
        }
...
template <class T>
double SVM_Vector_atonce_temp<T>::eTrainingVector(int i) const
{
    double res = 0;
...
template <class T>
Vector<double> &SVM_Vector_atonce_temp<T>::dedgTrainingVector(Vector<double> &res, int i) const
{
    ...
    if ( ( i < 0 ) || isenabled(i) )
    {
        ...
        {
            angle(res,res); // This should be fine.  angle first assigns the vector to the result, then does everything else on the result only, so it will be a trivial res = res followed by stuff of res.
        }
    }
...
template <class T>
Matrix<double> &SVM_Vector_atonce_temp<T>::dedKTrainingVector(Matrix<double> &res) const
{
    res.resize(N(),N());
...
template <class T>
Vector<double> &SVM_Vector_atonce_temp<T>::dedKTrainingVector(Vector<double> &res, int i) const
{
    NiceAssert( i < N() );
...
template <class T>
int SVM_Vector_atonce_temp<T>::gTrainingVector(Vector<double> &res, int &locclassrep, int i, int retaltg, gentype ***pxyprodi) const
{
    ...
    else if ( ( dtv & (7+64) ) )
    {
        ...
        if ( dtv > 0 )
        {
            ...
            if ( NF() )
            {
                for ( iP = 0 ; iP < NF() ; ++iP )
                {
                    K2(tempres,i,(Q.pivAlphaF()(iP)),pxyprodi ? (const gentype **) pxyprodi[Q.pivAlphaF()(iP)] : nullptr);
...
//    }
    ...
    {
//        fullOptStateSMOVect<T> xx(Q,*GpGrad,*Gpsigma,Gn,Gpn,gp,gn,hp,ub,maxitcntval,(int) maxtraintimeval,nullptr,nullptr,1,DEFZCMAXITCNT,DEFINMAXITCNT,Opttol(),zerotol(),zerotol(),biasV().size());
//
//        res = xx.wrapsolve(killSwitch);
        ...
        {
            fullOptStateSMOVect<T> xx(Q,*GpGrad,*Gpsigma,Gn,Gpn,gp,gn,hp,ub,nullptr,nullptr,1,DEFZCMAXITCNT,DEFINMAXITCNT,Opttol(),zerotol(),zerotol(),biasV().size());
...
int SVM_Vector_atonce_temp<T>::presolveit(const Vector<double> &betaGrad)
{
    ...
    for ( i = 0 ; i < N() ; ++i )
    {
        twoProduct(alphaProject("&",i),betaGradUnit,alphaV()(i));
...
std::ostream &SVM_Vector_atonce_temp<T>::printstream(std::ostream &output, int dep) const
{
    ...
    repPrint(output,'>',dep) << "End training time (sec):         " << traintimeendval        << "\n";
