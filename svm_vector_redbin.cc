...
template <>
double SVM_Vector_redbin<SVM_Scalar>::K2(int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo, int resmode) const;

template <>
gentype &SVM_Vector_redbin<SVM_Scalar>::K2(gentype &res, int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo, int resmode) const;

template <>
double SVM_Vector_redbin<SVM_Scalar>::K2ip(int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo) const;
...
void evalXYSVM_Vector_redbin_SVM_Scalar(double &res, int i, int j, const gentype **pxyprod, const void *owner)
{
    ...
    res = realOwner->K2ip(i,j,pxyprod);
...
double SVM_Vector_redbin<SVM_Scalar>::K2(int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo, int resmode) const
{
    return SVM_Generic::K2(i,j,pxyprod,xx,yy,xxinfo,yyinfo,resmode);
}

template <>
gentype &SVM_Vector_redbin<SVM_Scalar>::K2(gentype &res, int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo, int resmode) const
{
    return SVM_Generic::K2(res,i,j,pxyprod,xx,yy,xxinfo,yyinfo,resmode);
}
...
int SVM_Vector_redbin<SVM_Scalar>::settspaceDim(int newMinDim)
{
	...
	{
            db("&",tspacedimold,1,tspacedimnew-1,tmpva) = Q(0).biasR();
	}
	...
	if ( N() )
	{
		...
		if ( tspacedimnew > tspacedimold )
		{
                    dalpha("&",i)("&",tspacedimold,1,tspacedimnew-1,tmpva) = Q(0).alphaR()(i);
		}
	    }
	}
...
int SVM_Vector_redbin<SVM_Scalar>::addtspaceFeat(int iii)
{
	...
	// resize db and dalpha
        ...
        db.add(iii);
        db("&",iii) = db(0); // this is the value that will be set by the code used below
