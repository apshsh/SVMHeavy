...
//FIXME need option to return RKHSVector for SVM,LS-SVM,GPR,SSSVM
...
//FIXME: - put the -g... calls that set variables in globalopt etc in a function on their own.
//       - this function is given pointers to relevant optimisers - 4, one for each type of optimiser - and sets relevant variables only if pointer non-nullptr
//       - then *all* options for internal optimisation models can be set recursively (-gB { -g... }) will call the function with the relevant "extra" pointing to the internal optimiser to eg 
//         the internal multi-objective direct optimiser etc (make this recursive)
//       - this will also let you make all internal optimisers of arbitrary type!  (Use JIT allocate-on-request-for-reference-to-optimiser for these pointers to avoid infinite recursion)
//       - when function callback occurs, pass the current optimiser pointer (single pointer, so need type function for casting) back.  Then you can pass this to the function (eg -gL {-g... }) to set any internal stuff on-the-fly!
//
// See getModelErrOptim(, killModelErrOptim( in smboopt for example of how this should work, needs to also be implemented for various optimisers in bayesopt
...
THINGS TO FIX:
 ...
 - xact (see SSV_generic) needs to also incorporate d - if d = 0 don't include, make changes on setd
