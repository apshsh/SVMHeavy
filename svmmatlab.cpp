...
// Uncomment to prevent matlab doing system calls for plots
// (will leave .m files and data files to generate images later)
#define MEX_NOSYS
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    try
    {
        ...
        if ( !hasbeeninit )
        {
            ...
            svm_setatexitfn(mexAtExit); // this needs to go first

            //initgentype(); - not needed anymore
            ...
            static LoggingOstreamErr mexcerr(xmexCharPrintErr);
            ...
            static LoggingIstream mexcinPrompt(xmexCharRead);
            static LoggingOstreamOut mexcoutPrompt(xmexCharPrintOut);
            setpromptstream(&mexcinPrompt,&mexcoutPrompt);
        ...
        if ( ( argc == 2 ) && mxIsNumeric(prhs[0]) )
        {
            ...
            //double temp = mxGetDoubles(prhs[0]);
...
int mex_system(const char *command)
{
    (void) command;
...
#ifndef MEX_NOSYS
    // Like case -3 above, with numargs = 0
    ...
    ires = mexCallMATLAB(1,lhs,0,rhs,command);
...
//        if ( dstr && dsti )
        {
            *(reinterpret_cast<std::complex<double>*>(mxGetComplexDoubles(res))) = (std::complex<double>) srccnt;
...
//                if ( dstr && dsti )
                {
                    for ( i = 0 ; i < numRows ; i++ )
                    {
                        ...
                        (reinterpret_cast<std::complex<double>*>(mxGetComplexDoubles(res)))[i] = (std::complex<double>) ival;
...
//                double *dsti = mxGetPi(res);
                ...
                for ( i = 0 ; i < numRows ; i++ )
                {
                    for ( j = 0 ; j < numCols ; j++ )
                    {
                        ...
                        (reinterpret_cast<std::complex<double>*>(mxGetComplexDoubles(res)))[i+(j*numRows)] = (std::complex<double>) ival;
    ...
    if ( mxIsNumeric(src) )
    {
        ...
        else if ( ( mxGetNumberOfDimensions(src) == 2 ) && ( mxGetM(src) == 1 ) && ( mxGetN(src) == 1 ) && !dst.isValVector() && !dst.isValMatrix() )
        {
            if ( !mxIsComplex(src) )
            {
                ...
                //dst.force_double() = mxGetDoubles(src);
            }
...
//                if ( srcR && srcI )
                {
                    ...
                    dst.dir_anion() = *(reinterpret_cast<std::complex<double>*>(mxGetComplexDoubles(src)));
...
//                }
        ...
        else if ( ( mxGetNumberOfDimensions(src) == 2 ) && ( mxGetN(src) == 1 ) && !dst.isValMatrix() )
        {
            ...
            dst.force_vector().resize((int) mxGetM(src));
...
//                    if ( srcR && srcI )
                    {
                        for ( i = 0 ; i < mxGetM(src) ; i++ )
                        {
                            ...
                            dst("[]",i).dir_anion() = (reinterpret_cast<std::complex<double>*>(mxGetComplexDoubles(src)))[i];
...
//                    }
        ...
        else if ( ( mxGetNumberOfDimensions(src) == 2 ) && ( mxGetM(src) == 1 ) && !dst.isValMatrix() )
        {
            ...
            dst.force_vector().resize((int) mxGetN(src));
...
//                    }
        ...
        else if ( mxGetNumberOfDimensions(src) == 2 )
        {
            ...
            dst.force_matrix().resize((int) mxGetM(src),(int) mxGetN(src));
...
//                    if ( srcR && srcI )
                    {
                        for ( i = 0 ; i < mxGetM(src) ; i++ )
                        {
                            for ( j = 0 ; j < mxGetN(src) ; j++ )
                            {
                                ...
                                dst("[]",i,j).dir_anion() = (reinterpret_cast<std::complex<double>*>(mxGetComplexDoubles(src)))[i+(j*mxGetM(src))];
...
void mexCharPrint(char c, int mode, int src);
void mexCharPrint(char c, int mode, int src)
{
