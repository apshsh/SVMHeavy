...
//
// method: (ignored if impmeasu != nullptr)
...
//           appropriate output type.  If nullptr then the default scalar GPR
//           will be used.
// impmeasu: improvement measure function.  If set will be used instead
//           of EI/PI/whatever.  nullptr to use inbuilt method instead.
//
...
//
//
// Bayesian Optimisation arguments II (fnapprox GPR_Scalar, impmeasu nullptr):
//
// method: (ignored if impmeasu != nullptr)
//         0 = raw output (ignoring variance, just minimise f(x))
//         1 = EI (expected improvement)
...
GlobalOptions *setupBayesOptI(int method = 0,
                              ML_Base *fnapprox = nullptr,
                              IMP_Generic *impmeasu = nullptr,
                              int startpoints = 5,
...
public:
    ...
    OptProb(int xidim,
            ...
            void *xfnarg,
            int *xnumpts = nullptr,
            int *xdistMode = nullptr,
            int *xvarsType = nullptr)
    {
        NiceAssert( xidim >= 1 );
        NiceAssert( xodim >= 0 );
        NiceAssert( xxmin );
...
//          (set nullptr to not keep these results)
//                                                  
